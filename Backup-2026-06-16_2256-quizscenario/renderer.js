...
(function (g) {
  ...
  LTRenderer.prototype._build = function (mount) {
    ...
    // thin per-beat progress bar (how far through the lesson)
    var prog = el('div', 'ltp2-progress');
    prog.setAttribute('role', 'progressbar');
    prog.setAttribute('aria-label', 'Lesson progress');
    this.progFill = el('div', 'ltp2-progress-fill');
    ...
    // end-of-lesson cue (shown on the last beat) — ties into the app's Quiz step via opts.onComplete
    ...
    this.doneBtn = el('button', 'ltp2-btn ltp2-done-btn', 'Continue to the quiz →');
    this.done.appendChild(this.doneBtn);
    ...
    this.beats.forEach(function (b, idx) {
      ...
      d.setAttribute('aria-label', 'Go to beat ' + (idx + 1) + ' of ' + self.beats.length + ' (' + b.type.toLowerCase() + ')');
    ...
    this.doneBtn.addEventListener('click', function () { if (self.opts.onComplete) self.opts.onComplete(); });
  ...
  LTRenderer.prototype._setControls = function () {
    ...
    // progress bar: how far through the lesson
    if (this.progFill) this.progFill.style.width = (n > 1 ? ((this.i + 1) / n) * 100 : 100) + '%';
    var kids = this.dots.children;
    for (var k = 0; k < kids.length; k++) kids[k].classList.toggle('on', k === this.i);
    // end-of-lesson cue on the final beat (the Continue button only if the host wired onComplete)
    if (this.done) {
      this.done.classList.toggle('on', last);
      this.doneBtn.style.display = (last && this.opts.onComplete) ? '' : 'none';
    }
  };
  ...
  LTRenderer.prototype._anno = function (P, an, av) {
    ...
    if (an.kind === 'level') {
      ...
      if (an.label) {
        ...
        ctx.textAlign = al;
        this._haloText(an.label, tx, below ? my + 10 : my - 10);
      }
