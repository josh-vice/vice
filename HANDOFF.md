...
- `Backup-2026-06-17_1631-viz-palette/` — before the UX/UI palette-unification pass (`later¹⁰`).
- `Backup-2026-06-17_1710-p2p3-polish/` — before the P2/P3 tooltip/legend/pin pass (`later¹²`).
- `Backup-2026-06-18_0007-refactor-done/` — **LATEST = current working tree** (code only; audio regenerable).
  Post architecture-refactor (see the 2026-06-18 log entry). **Current cache versions (= what's live in
  `lt-index.html`):** NEW foundational modules (load BEFORE the engine): `lt-store.js 1.0.0`, `lt-ta.js 1.0.0`,
  `lt-tokens.js 1.0.0`, `lt-utils.js 1.0.0`.
  `lt-engine.js 3.111.0`, `lt-chartgen.js 1.1.0`, `lt-data.js 1.9.3`, `lt-data-course2.js 1.9.0`,
  `lt-data-course3.js 1.10.0`, `lt-data-course4.js 1.15.0`, `lt-settings.js 1.30.0`, `lt-sim-ui.js 1.15.12`,
  `lt-flashcards.js 1.9.4`, `lt-gallery.js 1.3.2`, `lessons-v2/renderer.js 1.14.0`,
  `lessons-v2/player.css 1.8.0`, `lt-community.js 1.11.1`, `lt-styles.css 1.98.3`.
- `Backup-2026-06-17_2258-refactor/` — pre-refactor restore point (last pure-chart-system state).
- `Backup-2026-06-17_2106-current/` — pre-refactor milestone (`lt-engine.js 3.106.0`).
...
### ★ Latest (2026-06-30) — Chart teaching-annotation pass COMPLETE → see `CHART_AUDIT_HANDOFF.md`
"Anything referenced while teaching must be drawn+labelled on the chart." All three tracks done, verified
(0 throws / 0 clipped / 0 console errors), deployed: **ECharts course charts** (all 81 chapters — 159 edits
total incl. C4 ch4,7–29 this round), **v2 animated lessons** (36 `show` adds across 21 of 55 visual-beat
lessons; 84 lessons · 397 beats render clean), **simulator** (verified). **Current cache versions live in
`lt-index.html`:** `lt-engine.js 3.114.1` (carries the y-axis clip-fix + the v2 lesson cache token `?v=1.0.1`),
`lt-data.js 1.9.5` · `lt-data-course2.js 1.10.0` · `lt-data-course3.js 1.11.0` · `lt-data-course4.js 1.18.0`.
Reusable tooling under `tools/chart-audit/` (appliers, audit workflows, vetted plans, `verify-sweeps.md`).
Only **optional** follow-ups remain (simulator `#cc2222`→`#ff2e88` palette; 40 v2 noAnchor gaps) — see that file.
The dated entries below pre-date this work.

### ★ Where things stand (2026-06-17, end of a long chart+UI day — `later⁷`…`later¹²`)
The **entire chart/visual system overhaul is DONE & verified** (0 console errors throughout, desktop+mobile).
In order: Hyblock tool visualizations rolled out across Courses 3&4 (`later⁷`) → realistic `ltCandles` +
lead-in/follow-through context (`later⁸`) → trader-audit fixes: volume panel, real CVD, E2E candle redesign,
sub-panel units, directional context (`later⁹`) → unified Vice-Terminal data-viz palette + legibility (`later¹⁰`)
→ lesson-complete overlay + community-page overhaul + landing number-logo fix & audit (`later¹¹`) → P2/P3 polish:
sub-panel tooltips, a11y, pin-density compaction, off-by-default colour-key toggle (`later¹²`). Each is a dated
log entry below. **No known open chart/UI bugs.** Latest tree = `Backup-2026-06-17_2106-current/`.

**Deploy:** OpenClaw auto-deploys to Vercel from the **working tree** — do NOT run a manual `vercel --prod`.
**Nothing is git-committed** (commit if you want git as a net); everything above lives only in local files.
`.vercelignore` ships `lessons-v2/**` runtime (JS + `player.css` + `audio/**`) and excludes `*.md` docs, the
`course*.html`/`preview.html` hubs, `tools/tts/.venv`, `rebuild`/`player`, `.anthropic_key`, `PRODUCT.md`, etc.

### ✅ Shipped today (full detail in the dated log below)
- **Hyblock tool viz + sub-panels** — liq-cluster ladders, amber positions-heatmaps, order-book depth, and
  funding/basis/OI/CVD/volume sub-panels drawn (not just named) across the relevant Course-3/4 charts.
- **Chart accuracy + realism** — richer `ltCandles` (counter-trend candles, varied wicks, vol-clustering),
  per-chart lead-in/follow-through context, real volume + a true CVD read, E2E now closes inside the Kumo.
- **One Vice-Terminal data-viz palette** — teal=bull/buy/support · pink=bear/sell/resistance · gold=liq ·
  amber=heatmap; contrast-aware price-tag pills; sub-panel scale+units+sign hints; a per-chart colour-key
  toggle (off by default) + hover tooltips that report the sub-panel value under the cursor.
- **UI** — lesson-complete overlay synced to the OSRS fanfare; community page rebuilt to match the app;
  landing hero "number logo" now scales properly (was a `transform` hack) + section-eyebrow cleanup.

### Open / optional work
1. **Quiz answer spot-check** — ~10 multiple-choice answers in `lt-module-quizzes.js` were set from domain
   knowledge (the PDF's letter markings didn't survive extraction). Worth the owner's eyeball (list in the
   "SCENARIO + per-module Quiz" entry below).
2. **(Optional) Ichimoku ch28** — the E2E candle redesign (`later⁹`) was scoped to **ch27** (the Edge-to-Edge
   chapter); **ch28 "Ichimoku Market Scenarios"** was left as general above/below-cloud cases (it doesn't claim
   E2E). Rework its candle paths too if you want them to demonstrate specific setups.
3. **(Optional) Landing hero gradient text** — the one `.grad` phrase ("one candle at a time") is a deliberate
   neon brand moment kept against the usual gradient-text ban; flip to a solid colour if you'd rather be strict.
4. **`impeccable` skill** installed (`.claude`/`.agents` + PostToolUse design hook; no `/impeccable` slash cmd —
   run via the Skill tool / reference docs). Open: `/impeccable document` → generate **DESIGN.md** (the token
   system + the new `VIZ` data-viz palette aren't captured anywhere yet).
5. **Simulator candles** inconsistent (`lt-sim-ui.js` solid-up vs hollow) — align to the site's
   hollow-up/filled-down + the unified teal/pink for full-app uniformity.
6. **(Optional)** Audio weight (~96MB/486 files at 96k — drop bitrate in `tools/tts/generate.py` if needed);
   glossary term-flash only scans the intro HTML; dead recon CSS (`.ltp-*`/`.chart-view-tabs`) lingers in
   `lt-styles.css` (harmless).

## Latest session (2026-06-18) — Architecture refactor: course registry · Store facade · TA/tokens/utils modules

Senior-engineer code-quality pass (owner: "reverse-engineer the architecture, then refactor for quality/
scalability/maintainability — do NOT change functionality"). **Behaviour-identical by construction**; verified
in the Claude Preview with 0 console errors and a 231-candlestick + 79-reveal + 6-line chart-build sweep (0
throws), live course-switch round-trip, and a localStorage round-trip matrix. Backups:
`Backup-2026-06-17_2258-refactor/` (pre) → `Backup-2026-06-18_0007-refactor-done/` (post). Cache:
**lt-engine.js 3.111.0**, lt-settings.js 1.30.0, lt-sim-ui.js 1.15.12, lt-flashcards.js 1.9.4, lt-gallery.js
1.3.2, + NEW `lt-store`/`lt-ta`/`lt-tokens`/`lt-utils` @ 1.0.0 (wired in lt-index.html, load BEFORE the engine).

1. **Course registry (`LT_COURSES` in lt-engine.js).** One source of truth for each course's
   {chapters, exam, stateKey, accent, accentLight}. Replaced the reverse-identity `getActiveCourseNum`, the 5
   course→X helpers (`getCourseStorageKey`/`ltCourseAccent`/`_examPool`/`_ltCourseArray`/`_ltCourseKey`), the 3
   `CHAPTERS=` reassignment ladders, the 4 inline `lt_courseN_state` ternaries, and the 3 partial list-copies
   (sidebar accordion / exam answer-key / home cards). **Adding Course 5 = one row + one `<script>`.**
   `LT_CHAPTERS_2/3/4` now appear only in the registry + the one boot-time `_expandAllChartContext` iteration.
   Display NAME strings stayed at their call sites (sidebar/exam/home use 3 different conventions). NOTE:
   `_ltCourseChapters` in lt-settings.js is a 4th settings-local course→chapters ladder left as-is (settings
   loads before the engine, can't read LT_COURSES at parse).

2. **Persistence facade (`lt-store.js` → `LTStore` + `LT_KEYS`).** One registry of the 15 fixed localStorage
   keys + safe get/set/del/json (+ getRaw/setRaw/delRaw for dynamic keys). Migrated all fixed-key calls in
   engine/settings/sim-ui/flashcards/gallery. **Key STRINGS unchanged — a compatibility contract** (sync/backup
   sweep the whole `lt_` namespace; learners have existing data). LEFT RAW on purpose: the generic `lt_`-prefix
   sweepers (export/import/sync/reset keep-list), the per-course `lt_courseN_state` + `lt_examDone_<n>` dynamic
   keys, and **lessons-v2/renderer.js** (deliberately dependency-free; reads lt_lesson_speed/volume directly).

3. **TA module (`lt-ta.js` → `LTTa`).** Extracted the engine indicator math (ichimokuMid, computeIchimoku,
   synthIchiLookback, computeRSI, cumDelta, synthVolume + shared ICHI_DEFAULT). The engine `_compute*`/`_synth*`
   are thin delegators (call sites unchanged); `_ICHI_DEFAULT = LTTa.ICHI_DEFAULT` (still read by
   `_applyIchimokuOverlay`). Output verified byte-identical. **The v2 canvas lessons keep their OWN crypto-tuned
   Ichimoku/RSI (chart-vocabulary.js) — intentionally NOT merged** (different periods → different visuals).

4. **Token module (`lt-tokens.js` → `LT_TOKENS`).** Canonical data-viz palette; engine `VIZ`/`RED`/`GOLD`/`TEAL2`
   alias to it (19 `VIZ.*` refs unchanged). `TEAL`/`BEAR` stay in the engine (mutable, theme-aware).

5. **Utils module (`lt-utils.js` → `LTUtils`).** Pure colour/format helpers (hexToRgb, pillText, fmtCompact,
   legibleOnDark) extracted; engine helpers delegate. lt-engine.js 5290 → 5186 lines.

### Refactor follow-ups (deliberately NOT done — deferred from the overnight run)
- **Deep engine split** (chart-builder / exam / sidebar / nav clusters → own files): DOM/state-coupled, large,
  interdependent — too risky to do blind on an auto-deploying prod site. Do attended / in a branch. ta+tokens+
  utils already peeled the clean pure clusters out of the monolith.
- **Colour-token drift**: renderer.js still hardcodes `#cc4444`/`rgba(204,68,68)`; ~772 hardcoded colour
  literals remain in lt-data*.js. Pointing these at LT_TOKENS would CHANGE rendered visuals → out of scope for
  a quality-only pass. Do behind a visual-diff check.
- **Cross-renderer TA merge** (engine `LTTa` vs the canvas vocabulary): gated on confirming identical output.
