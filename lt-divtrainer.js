...
(function () {
  ...
  function _dtStyles() {
    LTUtils.injectStyles('lt-divtrainer-styles', [
      ".dt-wrap{width:100%;max-width:640px;margin:0 auto;font-family:'Geist Mono',ui-monospace,monospace;color:var(--text,#f6f5fb);}",
  ...
  function _panelSvg(sc) {
    ...
    var guides = [70, 50, 30].map(function (v) {
      return '<line x1="' + PAD_L + '" y1="' + RY(v).toFixed(1) + '" x2="' + (W - PAD_R + 26) + '" y2="' + RY(v).toFixed(1) + '" stroke="' + S + '" stroke-width="' + (v === 50 ? 0.6 : 1) + '" stroke-dasharray="3 4" opacity=".4"/>'
        + '<text x="' + (W - PAD_R + 30) + '" y="' + (RY(v) + 3).toFixed(1) + '" fill="' + S + '" font-size="8.5" font-family="Geist Mono,monospace" opacity=".8">' + v + '</text>';
    }).join('');
    ...
    return {
      ...
      svg: '<svg viewBox="0 0 ' + W + ' ' + H + '" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Price and RSI panels">'
        ...
        + '<text x="' + PAD_L + '" y="' + (rTop - 1) + '" fill="' + S + '" font-size="8.5" letter-spacing="1" font-family="Geist Mono,monospace">RSI · ' + RSI_P + '</text>'
        + guides
