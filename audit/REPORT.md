...
**Addendum 3 — owner feature wave (hub.js v2.16.0 / hub.css v1.32.0 / vchart.js v1.9.1):** Focus board recomposed (HL Positioning into the right rail under the quote; flush Funding+Countdown bottom band); Scanner symbol scope (Focus on ETH → ETH traders / positions / sentiment, `#/scanner/ETH` deep links, clearable chip, scoped columns and aggregate strip); CME Gap Tracker as both a widget and a `<Vice>` chart indicator (Chicago-time halt/reopen detection on hourly history, unfilled gaps shaded until price fills them). A 4-agent adversarial pass on the navigation wave confirmed 13 defects — all fixed: mutually-exclusive bar menus that close on every transition, viewport-edge flip + height cap + menu roles on the switcher, decode+round-trip-guarded chart deep links, and one trust rule for section symbols. Verified live throughout; zero console errors.

**Addendum 2 — UI/UX + navigation overhaul (hub.js v2.15.0 / hub.css v1.31.0):** the design-lens pass identified the native layout `<select>` as the last OS-styled control in the product — replaced with a designed switcher menu (active check, 1–5 kbd hints, New layout…, Esc-aware, one-menu-at-a-time, name survives mobile). Chart page now carries its symbol in the URL (`#/chart/ETH` shares and reloads correctly; fixed a boot race where a null feed snapshot sent deep links to BTC). Hub footer got the family flamingo mark. Focus board recomposed per owner: HL Positioning moved from a stretched bottom row into the right rail directly under Symbol Info, with the Hyperliquid chart below it and a flush full-width Funding+Candle-Close bottom band (Funding at 14 cols now shows all five venue columns). Site-nav verdicts that needed no change are documented in ADDITIONS.md §15-17.
