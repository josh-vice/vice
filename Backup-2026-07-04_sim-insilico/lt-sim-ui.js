...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   • Trader UX: OHLC crosshair tooltip, scroll-back/zoom with a "Latest"
     re-pin, bar-replay Step button (→ key), $risk / R:R readout, size-by-1%%
     -risk chip, liquidation-proximity warning, blown-account state.
...
window._sim3SizeRisk = function(pct) {
  ...
  const el = _el('sim3-size');
  if (el) el.value = Math.max(0, Math.floor(usd)).toString();
  _paintOrderPanel();
};
...
function _simStyles() {
  ...
  .sim3-blown { padding:14px; border-bottom:1px solid var(--border2); background:rgba(255,46,136,.06); }
  .sim3-blown-title { font-size:12px; font-weight:800; color:#ff5f8f; letter-spacing:.4px; margin-bottom:4px; }
  .sim3-blown-sub { font-size:11px; color:var(--text2); line-height:1.5; margin-bottom:10px; }
  .sim3-blown-btn { width:100%; padding:9px; border:1px solid #ff2e88; border-radius:9px; background:rgba(255,46,136,.12); color:#ff5f8f; font-size:12px; font-weight:800; cursor:pointer; font-family:inherit; }
      ...
      <div class="sim3-term">
        ...
        <div class="sim3-fields">
          ...
          <div class="sim3-frow">
            ...
            <input class="sim3-input" id="sim3-size" type="number" placeholder="Notional in USD" aria-label="Order size in US dollars" oninput="_paintOrderPanel && _paintOrderPanel()" />
