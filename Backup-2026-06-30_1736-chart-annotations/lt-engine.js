...
function _applyIchimokuOverlay(opt, def, ctx) {
  ...
  // cloud colour by net bias (ECharts area fill is single-colour per series)
  let up = 0, dn = 0;
  for (let i = 0; i < ik.spanA.length; i++) {
    if (ik.spanA[i] != null && ik.spanB[i] != null) (ik.spanA[i] >= ik.spanB[i] ? up++ : dn++);
  }
  const bull = up >= dn;
  const fill  = bull ? 'rgba(40,200,120,0.20)' : 'rgba(242,61,92,0.18)';
  ...
  // Kumo fill: stacked area (base = min(A,B) transparent, fill = |A-B| coloured)
  const base  = ik.spanA.map((a, i) => a != null && ik.spanB[i] != null ? Math.min(a, ik.spanB[i]) : null);
  const delta = ik.spanA.map((a, i) => a != null && ik.spanB[i] != null ? Math.abs(a - ik.spanB[i]) : null);
  opt.series.push({ name: '_kumo_base', type: 'line', data: base,  stack: 'kumo', symbol: 'none', silent: true, showSymbol: false, lineStyle: { opacity: 0 }, areaStyle: { color: 'transparent' }, z: 0 });
  opt.series.push({ name: '_kumo_fill', type: 'line', data: delta, stack: 'kumo', symbol: 'none', silent: true, showSymbol: false, lineStyle: { opacity: 0 }, areaStyle: { color: fill }, z: 0 });
...
function buildCandlestickOption(def, revealMode) {
  ...
  const _opt = {
    ...
    yAxis: {
      scale:     true,
      // Expand (never shrink) the auto extent to include any off-candle teaching level.
      min: _lvlLo == null ? undefined : (v) => { const pad = ((v.max - v.min) || 1) * 0.05; return Math.min(v.min, _lvlLo - pad); },
      max: _lvlHi == null ? undefined : (v) => { const pad = ((v.max - v.min) || 1) * 0.05; return Math.max(v.max, _lvlHi + pad); },
      splitNumber: sm ? 4 : 5,
...
function ensureV2LessonsLoaded(n) {
  ...
  _v2LoadState[n] = Promise.all(files.map((f) => new Promise((resolve) => {
    ...
    s.src = `lessons-v2/lessons/Course${n}/${f}?v=1.0.0`;
