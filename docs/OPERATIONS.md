...
For runtime verification, use isolated ports when another local service may be running:

```bash
VICE_BACKEND_PORT=18080 VICE_FRONTEND_PORT=15173 VICE_SERVICE_ID=vice-release bun run dev
VICE_BACKEND_URL=http://127.0.0.1:18080 VICE_FRONTEND_URL=http://127.0.0.1:15173 bun run smoke
VICE_FRONTEND_URL=http://127.0.0.1:15173 bun run test:browser
```

The launcher must clean up both services. Confirm the chosen ports are free after `Ctrl+C`.
