...
- The explicit port manifest maps all six widget identities to US-014 and declares their provenance. Coverage: `suite/widgets.test.js`, native-port surface and read-only chart tests, existing watchlist and price-alert tests, widget catalog/migration tests, source typecheck, Svelte check, and production build.
