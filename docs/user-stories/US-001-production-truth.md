...
- Book snapshot boundary: the current Hyperliquid L2 payload has no sequence field, so the terminal cannot truthfully claim sequence-gap reconstruction for it. It now rejects empty, malformed, unordered, or crossed snapshots before committing them to the ladder, marks market data stale, and runs the existing recovery path. Automated coverage: `src/lib/hl/normalize.test.js` and `src/lib/hl/subscriptionsIdentity.test.js`. Worker-side book parsing and authoritative sequence support remain open.
