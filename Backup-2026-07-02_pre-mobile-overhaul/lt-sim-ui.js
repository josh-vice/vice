...
/* Round a rough interval up to a "nice" tick step (1/2/5 × 10^n) so axis
   bounds snap to clean values instead of raw data extents. */
function _sim2NiceStep(rough) {
  const p = Math.pow(10, Math.floor(Math.log10(Math.abs(rough) || 1)));
  const n = rough / p;
...
let _simSetupRO     = null;   // ResizeObserver for the setup chart (disconnected on teardown)
let _simEquityRO    = null;   // ResizeObserver for the equity chart
...
function _bc() {
  return (typeof getBullishColor === 'function') ? getBullishColor()
    : (LTStore.get('bullishColor') || '#00d4d4');
}
function _bear() {
  return (typeof getBearishColor === 'function') ? getBearishColor()
    : (LTStore.get('bearishColor') || '#f2f2f2');
}
...
/* ── STYLES ───────────────────────────────────────────────────────────────── */
function _simStyles() {
  LTUtils.injectStyles('lt-sim2-styles', `
  /* ── layout ── */
  ...
  .sim2-pat-select { background:var(--bg4); border:1px solid var(--border2); border-radius:8px; color:var(--text2); font-family:'JetBrains Mono',monospace; font-size:11px; font-weight:600; padding:5px 8px; cursor:pointer; max-width:240px; text-overflow:ellipsis; }
  ...
  @media(max-width:600px) {
    ...
    .sim2-pat-select { max-width:min(64vw, 240px); }
...
function _sim2RenderEquityChart() {
  ...
  if (_simEquityRO) { try { _simEquityRO.disconnect(); } catch(_) {} }
  _simEquityRO = new ResizeObserver(()=>{ try{_simEquityChart&&_simEquityChart.resize();}catch(_){} });
  _simEquityRO.observe(el);
}
...
function _sim2RenderSetup() {
  ...
  _simChartInst.setOption({
    ...
    // lowest, so price is never flush to an edge. Bounds snap to a nice tick
    // step so ECharts never prints raw-extent boundary labels ("33,253.08").
    yAxis:{
      scale:true, position:'left',
      min:(v)=>{ const r=(v.max-v.min)||1; const s=_sim2NiceStep(r*1.4/5); return Math.floor((v.min - r*0.2)/s)*s; },
      max:(v)=>{ const r=(v.max-v.min)||1; const s=_sim2NiceStep(r*1.4/5); return Math.ceil((v.max + r*0.2)/s)*s; },
    ...
    series:[{
      ...
      itemStyle:{color:'transparent',color0:_bear(),borderColor:TEAL,borderColor0:_bear(),borderWidth:1.5},
      markLine:{symbol:['none','none'],silent:true,data:mlData}
    }]
  });
  // Draw the entry / SL / TP order lines on top once layout is ready.
  _sim2DrawOrderLines();
  if (_simSetupRO) { try { _simSetupRO.disconnect(); } catch(_) {} }
  _simSetupRO = new ResizeObserver(()=>{ try{ if(_simChartInst){ _simChartInst.resize(); if(!_simAnswered) _sim2DrawOrderLines(); } }catch(_){} });
  _simSetupRO.observe(el);
}
...
function renderSimulator(containerId, opts) {
  ...
  const patternOptions = Object.keys(byCourse).map(course => {
    const opts = byCourse[course].map(k => {
      ...
      return `<option value="${k}"${sel} title="${SIM_CONCEPTS[k].name}">${nm}</option>`;
    }).join('');
...
/* Full teardown of live sim resources — safe to call any time (idempotent).
   Called on re-entry (renderSimulator) and by the engine's disposeAllCharts()
   whenever the user navigates away, so no reveal interval, chart instance, or
   ResizeObserver survives leaving the simulator view. */
function ltSimTeardown() {
  ...
  if (_simSetupRO)     { try { _simSetupRO.disconnect();  } catch(_) {} _simSetupRO     = null; }
  if (_simEquityRO)    { try { _simEquityRO.disconnect(); } catch(_) {} _simEquityRO    = null; }
}
