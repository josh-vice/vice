...
const LT_CHAPTERS_4 = [
     ...
     ═══════════════════════════════════════════════════════════════════════ */
  {
    ...
    lessonChart: {
      ...
      markLines: [ { yAxis: 113, label: "Stop — Above Fake-Breakout Wick", color: "#ff2e88" },
     ...
     ═══════════════════════════════════════════════════════════════════════ */
  {
    ...
    lessonChart: {
      ...
      type: "candlestick",
      // Pool candle CLOSES above Range Low (88) with only its wick piercing ~80 — a clean wick-and-reclaim
      ohlc: ltCandles(90, [
        { to: 87, bars: 9 },
     ...
     ═══════════════════════════════════════════════════════════════════════ */
  {
    ...
    // Bespoke interactive section (its own step after the Lesson) — the animated
    // Sentiment Board (lt-sentiment.js): the four SA signals aligning into a squeeze.
    // Charts untouched.
    demo: {
      kind: "sentiment",
      label: "Sentiment Board",
      after: "lesson",
      heading: "When All Four Signals Agree",
      body: "No single sentiment reading is a trade. The edge is <strong>confluence</strong> — funding, open interest, cumulative delta and basis all reaching extremes at the same key level. Watch the four line up at a DBS support: the crowd is trapped short, and the squeeze becomes the high-conviction long.",
      bullets: [
        "Each signal moving alone is noise — <strong>all four at once</strong>, at a key level, is signal",
        "Crowded shorts (negative funding + rising OI + red delta + backwardation) at support = fuel for a squeeze UP",
        "The conviction meter is the whole point: size up only as more variables align",
        "Sentiment tells you WHO is trapped; your TA level tells you WHERE they get squeezed"
      ]
    },
    ...
    lessonChart: {
      ...
      markPoints: [
        { dataIndex:  7, label: "Only 2/4 SA — Enter Small",   position: "bottom" },
        { dataIndex: 11, label: "Win — But Not Max Size",      position: "top"    }
      ]
    },
     ...
     ═══════════════════════════════════════════════════════════════════════ */
  {
    ...
    lessonChart: {
      ...
      // Scorecard shows the count that scales size: only 2 of 4 SA aligned = half size.
      // The engine renders one sub-panel (funding, mildly aligned); OI/Δ conflict-or-flat.
      markAreas: [
        { y0: 81, y1: 85, label: "2/4 SA: Funding ✓ Δ ✓ · OI ✗ Basis ✗ → HALF size", color: "rgba(0,212,212,0.06)" }
      ],
      subPanel: { kind: "funding", label: "Funding (mild — 1 of the 2)", points: [[0,-0.01],[6,-0.045],[9,-0.03],[11,0],[15,0.012]] },
      markPoints: [
        { dataIndex:  8, label: "Only 2/4 SA — Half Size Entry", position: "bottom" },
        { dataIndex: 15, label: "Target Hit — Smaller Winner",   position: "top"    }
      ]
    },
...
const LT_EXAM_QUESTIONS_4 = [
  ...
  { chapterTitle: 'Liquidity Engineering',
    ...
    chart: { type:'candlestick', labels: ltLabels(12),
      ohlc: ltCandles(110, [{ to:94, bars:5 }, { to:98, bars:2 }, { to:94, bars:2 }], { seed: 361, wick: 0.4 }).concat([[94, 95, 87, 96],[95, 100, 94, 101],[100, 104, 99, 105]]),
      markLines: [{ yAxis: 94, label: 'Support', color: '#ff2e88' }],
      markPoints: [{ dataIndex: 9, label: 'Sweep + Reclaim', position: 'bottom', color: '#00d4d4' }] },
    answers: [
  ...
  { chapterTitle: 'Swing Failure Pattern',
    ...
    chart: { type:'candlestick', labels: ltLabels(12),
      // Prior swing high is a real PEAK tagging ~104, several bars before the SFP; then the SFP
      // candle wicks well above (to 111) and closes back below 104, with a bearish trail
      ohlc: ltCandles(86, [{ to:103, bars:4 }, { to:95, bars:3 }, { to:100, bars:2 }], { seed: 363, wick: 0.4 }).concat([[100, 103, 100, 111],[103, 98, 97, 104],[98, 93, 92, 99]]),
      markLines: [{ yAxis: 104, label: 'Prior Swing High', color: '#ff2e88' }],
      markPoints: [{ dataIndex: 3, label: 'Prior Swing High', position: 'top', color: '#ff2e88' }, { dataIndex: 9, label: 'SFP — Wick + Close Below', position: 'top', color: '#ff2e88' }] },
    answers: [
  ...
  { chapterTitle: 'Stops as Liquidity',
    ...
    chart: { type:'candlestick', labels: ltLabels(11),
      // EXACTLY two candles wick to ~100 (matched peaks in the WICKS, bodies ~97), several bars
      // apart with a clean pullback between; no other candle's high exceeds ~98
      ohlc: [
        [90, 92, 89, 93], [92, 95, 91, 96], [95, 97, 94, 98],
        [97, 97.5, 96.4, 100], /* equal high #1 */
        [97.5, 95, 94, 98], [95, 93, 92, 96], [93, 94.2, 92.3, 95],
        [94.2, 96.5, 93.6, 97.5],
        [96.5, 97, 95.6, 100], /* equal high #2 */
        [97, 95, 94, 98], [95, 93, 92, 96]
      ],
      markLines: [{ yAxis: 100, label: 'Equal Highs', color: '#ff2e88' }],
      markPoints: [{ dataIndex: 3, label: 'Equal High #1', position: 'top', color: '#ff2e88' }, { dataIndex: 8, label: 'Equal High #2', position: 'top', color: '#ff2e88' }] },
    answers: [
  ...
  { chapterTitle: 'Market Structure Shift',
    ...
    chart: { type:'candlestick', labels: ltLabels(11),
      // Clean lower-high staircase (108 → 104) with lower lows; after the first swing high, ALL
      // wicks stay at/below the marked 104 last-lower-high until ONE strong candle closes above it
      ohlc: [
        [98, 102, 97, 103], [102, 108, 101, 109], /* first swing high 108 */ [108, 102, 101, 108],
        [102, 97, 96, 102.5], [97, 95, 94, 98],
        [95, 103, 94.5, 104], /* last lower high — wick tags 104 */
        [103, 98, 97, 103.5], [98, 94, 93, 98.5],
        [94, 99, 93, 100],
        [99, 107, 98, 108], /* CHoCH — closes above 104 */
        [107, 110, 106, 111]
      ],
      markLines: [{ yAxis: 104, label: 'Last Lower High', color: '#00d4d4' }],
      markPoints: [{ dataIndex: 1, label: 'Swing High', position: 'top', color: '#8a8f99' }, { dataIndex: 5, label: 'Last Lower High', position: 'top', color: '#8a8f99' }, { dataIndex: 9, label: 'CHoCH — Breaks Last LH', position: 'top', color: '#00d4d4' }] },
    answers: [
