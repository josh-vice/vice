...
(window.LT_LESSONS = window.LT_LESSONS || {})['course1/12_risk_management'] = {
  ...
  beats: [
    ...
    },
    {
      ...
      say: "Before any trade, define three prices: your entry, your stop-loss, and your target. The stop caps what you can lose; the target is what you expect to earn. Every trade you take starts as these three lines on a chart.",
      show: [
    ...
    },
    {
      ...
      chart: 'trade_setup_long',
      stage: 'plan',
      say: "The distance from entry to stop is your risk — one unit, one R. The distance from entry to target is your reward. Here the reward is twice the risk: a two-to-one ratio. That ratio — your risk-to-reward ratio, Triple R — is how you decide whether a setup is even worth taking, before you risk a cent.",
      show: [
    ...
    },
    {
      ...
      say: "Now watch it play out. Price dips toward the stop but never hits it, then runs up and reaches the target — a clean two-R winner. But say a couple of indecision candles had spooked you into closing early at one-and-a-half R: you expected two R, your actual result — the R-multiple — is 1.5. The expected ratio is your plan; the R-multiple is the postmortem.",
      show: [
    ...
    },
    {
      id: 'sizing',
      type: 'CONCEPT',
      say: "Now the part that keeps you alive: position sizing. Your size on any trade is your equity, times the percent you're willing to risk, divided by your stop-loss percentage. Ten thousand dollars, risking one percent, with a five percent stop: that's two thousand contracts. Keep risk between one and five percent per trade. At one percent, you could lose a hundred trades in a row before going broke — a hundred chances to recover. Go all-in instead, and a single loss can end you.",
      panel: {
        title: 'Position Sizing',
        lines: [
          'Size = **equity × risk% ÷ stop-loss %**',
          '$10,000 × 1% risk ÷ 5% stop = **2,000 contracts**',
