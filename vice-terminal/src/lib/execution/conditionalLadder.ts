...
async function tick(id: string): Promise<void> {
	...
	if (!running.has(id)) return;
	// Do not infer completion from a missing local row. A child can still be
	// open/partially filled, while an unknown outcome must remain recoverable.
	if (outcomes.some((outcome) => outcome.status === 'open')) return;
	if (outcomes.some((outcome) => outcome.status === 'unknown')) {
		save(transitionLocalAlgoJob(job, 'paused', 'Conditional ladder children disappeared without an authoritative outcome'));
		stop(id);
		return;
	}
	...
	const filled = Math.min(job.size, observedFilled);
...
function startTimer(id: string): void {
	...
	running.add(id);
	timers.set(id, setInterval(() => void tickGuard.run(id, () => tick(id)).catch((error) => { const job = loadLocalAlgoJobs('conditional_ladder').find((candidate) => candidate.id === id); if (job?.status === 'running') save(transitionLocalAlgoJob(job, 'failed', error instanceof Error ? error.message : 'Conditional ladder reconciliation failed')); stop(id); }), 250));
...
export async function startConditionalLadder(market: MarketDescriptor, params: ConditionalLadderParams): Promise<{ ok: boolean; jobId?: string; error?: string }> {
	...
	try {
		...
		const current = loadLocalAlgoJobs('conditional_ladder').find((candidate) => candidate.id === job.id);
		if (current?.status === 'running') save(transitionLocalAlgoJob(current, 'paused', error instanceof Error ? error.message : 'Conditional ladder could not start'));
		stop(job.id);
	}
