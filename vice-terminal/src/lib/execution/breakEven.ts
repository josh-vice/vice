...
function stop(id: string): void { const timer = timers.get(id); if (timer) clearInterval(timer); timers.delete(id); running.delete(id); lastStatusCheck.delete(id); }
...
async function tick(id: string): Promise<void> {
	...
	if (job.currentOrderId) {
		...
		if (Date.now() - (lastStatusCheck.get(id) ?? 0) >= 1_500) {
			lastStatusCheck.set(id, Date.now());
			const outcome = await localExecution.getOrderOutcome(currentOrderId);
			if (!running.has(id)) return;
			if (outcome.status === 'filled' || outcome.filled >= job.size) { save(transitionLocalAlgoJob(job, 'completed')); if (job.deadmanMs) await localExecution.clearDeadman(); stop(id); return; }
			if (outcome.status === 'closed') {
				...
				save(transitionLocalAlgoJob({ ...job, currentOrderId: undefined, size: remainingSize }, 'paused', outcome.filled > 0
					? 'Break-even child partially filled and closed; review the residual position before resuming'
					: 'Break-even child closed without an authoritative fill'));
				stop(id);
				return;
			}
			// The local open-order projection may lag an authoritative partial
			// child. Never replace or complete protection while it is still open.
			if (outcome.status === 'open') return;
		}
		let current = get(openOrders).find((order) => order.id === currentOrderId);
		if (!current) { await import('$lib/hl/orders').then(({ fetchOpenOrders }) => fetchOpenOrders()); if (!running.has(id)) return; current = get(openOrders).find((order) => order.id === currentOrderId); }
		if (current) return;
		const outcome = await localExecution.getOrderOutcome(currentOrderId);
		if (!running.has(id)) return;
		if (outcome.status === 'filled' || outcome.filled >= job.size) {
			...
			const remainingSize = Math.max(0, job.size - outcome.filled);
			save(transitionLocalAlgoJob({ ...job, currentOrderId: undefined, size: remainingSize }, 'paused', outcome.filled > 0
				? 'Break-even child partially filled and disappeared; review the residual position before resuming'
				: 'Break-even child disappeared after a terminal cancellation'));
		} else {
			save(transitionLocalAlgoJob(job, 'paused', 'Break-even child disappeared without an authoritative fill'));
		}
		stop(id); return;
	}
	...
	if (!running.has(id)) return;
	if (job.deadmanMs) { const armed = await localExecution.armDeadman(job.deadmanMs); if (!running.has(id)) { if (armed.accepted) await localExecution.clearDeadman(); return; } if (!armed.accepted) { save(transitionLocalAlgoJob(job, 'failed', armed.error ?? 'Dead-man switch could not be armed before break-even child')); stop(id); return; } }
	...
	save(placed);
	if (placed.deadmanMs) { const armed = await localExecution.armDeadman(placed.deadmanMs); if (!running.has(id)) { if (armed.accepted) await localExecution.clearDeadman(); return; } if (!armed.accepted) { save(transitionLocalAlgoJob(placed, 'failed', armed.error ?? 'Dead-man switch could not be re-armed')); stop(id); } }
}
function startTimer(id: string): void { if (timers.has(id)) return; running.add(id); timers.set(id, setInterval(() => void tickGuard.run(id, () => tick(id)).catch((error) => { const job = loadLocalAlgoJobs('break_even').find((candidate) => candidate.id === id); if (job?.status === 'running') save(transitionLocalAlgoJob(job, 'failed', error instanceof Error ? error.message : 'Break-even tick failed')); stop(id); }), 250)); void tickGuard.run(id, () => tick(id)); }
...
export async function startBreakEven(market: MarketDescriptor, params: BreakEvenParams): Promise<{ ok: boolean; jobId?: string; error?: string }> {
	...
	try {
		...
		const current = loadLocalAlgoJobs('break_even').find((candidate) => candidate.id === job.id);
		if (current?.status === 'running') save(transitionLocalAlgoJob(current, 'paused', error instanceof Error ? error.message : 'Break-even could not start'));
		stop(job.id);
	}
	const started = loadLocalAlgoJobs('break_even').find((candidate) => candidate.id === job.id);
