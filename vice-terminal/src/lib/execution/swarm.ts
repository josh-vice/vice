...
async function tick(id: string): Promise<void> {
	...
	const outcomes = await Promise.all(job.childOrderIds.map((orderId) => import('./localExecution').then(({ localExecution }) => localExecution.getOrderOutcome(orderId))));
	if (!running.has(id)) return;
	// A partial fill is still an open child and cannot be counted as a
	// completed ladder leg. Unknown outcomes are retained for explicit review.
	if (outcomes.some((outcome) => outcome.status === 'open')) return;
	if (outcomes.some((outcome) => outcome.status === 'unknown')) {
		save(transitionLocalAlgoJob(job, 'paused', 'Swarm children disappeared without an authoritative outcome'));
		stop(id);
		return;
	}
	const completelyFilled = outcomes.length === job.childOrderIds.length && outcomes.every((outcome) => outcome.status === 'filled');
	if (completelyFilled && job.childOrderIds.length > 0) {
...
function startTimer(id: string): void {
	...
	timers.set(id, setInterval(() => void tickGuard.run(id, () => tick(id)).catch((error) => {
		const job = loadLocalAlgoJobs('swarm').find((candidate) => candidate.id === id);
		if (job?.status === 'running') save(transitionLocalAlgoJob(job, 'failed', error instanceof Error ? error.message : 'Swarm reconciliation failed'));
		stop(id);
	}), 500));
...
async function placeRemaining(market: MarketDescriptor, job: LocalSwarmJob): Promise<LocalSwarmJob> {
	if (!running.has(job.id)) return loadLocalAlgoJobs('swarm').find((candidate) => candidate.id === job.id) ?? job;
	...
	if (job.deadmanMs) {
		...
		if (!running.has(job.id)) {
			...
			return loadLocalAlgoJobs('swarm').find((candidate) => candidate.id === job.id) ?? job;
		}
		if (!deadman.accepted) return saveAndReturn(transitionLocalAlgoJob(job, 'failed', deadman.error ?? 'Dead-man switch could not be armed before child placement'));
	}
	let placed = job;
	for (let index = placed.slicesPlaced; index < prices.length; index += 1) {
		if (!running.has(job.id)) return loadLocalAlgoJobs('swarm').find((candidate) => candidate.id === job.id) ?? placed;
		...
		if (!running.has(job.id)) {
			...
			return loadLocalAlgoJobs('swarm').find((candidate) => candidate.id === job.id) ?? placed;
		}
	...
	if (placed.deadmanMs) {
		...
		if (!deadman.accepted) return saveAndReturn(transitionLocalAlgoJob(placed, 'failed', deadman.error ?? 'Dead-man switch could not be armed'));
	}
	return placed;
}
...
export async function startSwarm(market: MarketDescriptor, params: SwarmParams): Promise<{ ok: boolean; jobId?: string; error?: string }> {
	...
	running.add(job.id);
	save(job);
	let placed: LocalSwarmJob;
	try {
		placed = await placeRemaining(market, job);
	} catch (error) {
		const current = loadLocalAlgoJobs('swarm').find((candidate) => candidate.id === job.id);
		if (current?.status === 'running') save(transitionLocalAlgoJob(current, 'paused', error instanceof Error ? error.message : 'Swarm could not start'));
		stop(job.id);
		return { ok: false, error: error instanceof Error ? error.message : 'Swarm could not start' };
	}
	if (placed.status !== 'running' || placed.slicesPlaced !== placed.slicesTotal) { stop(job.id); return { ok: false, error: placed.error ?? 'Swarm child placement failed' }; }
...
export async function resumeSwarm(id: string): Promise<void> {
	const job = loadLocalAlgoJobs('swarm').find((candidate) => candidate.id === id);
	if (!job || !['paused', 'failed'].includes(job.status) || job.restartRecoveryRequired) return;
	...
	if (get(selectedMarket)?.marketKey !== recovered.marketKey) {
		save(transitionLocalAlgoJob(recovered, 'paused', 'Select the Swarm market before explicitly resuming remaining slices'));
		return;
	}
	const market = get(marketRegistry).find((candidate) => candidate.marketKey === recovered.marketKey);
	if (!market) return;
	running.add(id);
	let complete: LocalSwarmJob;
	try {
		complete = await placeRemaining(market, { ...recovered, status: 'running', error: undefined, updatedAt: Date.now() });
	} catch (error) {
		const current = loadLocalAlgoJobs('swarm').find((candidate) => candidate.id === id);
		if (current?.status === 'running') save(transitionLocalAlgoJob(current, 'paused', error instanceof Error ? error.message : 'Swarm resume failed'));
		stop(id);
		return;
	}
	if (complete.status === 'running' && complete.slicesPlaced === complete.slicesTotal) startTimer(id); else stop(id);
}
export async function cancelSwarm(id: string, emergency = false): Promise<void> {
	...
	// The account snapshot can lag while a multi-child dispatch is still
	// settling. Cancel every known child, not only the rows currently present
	// in the local open-order store, so a just-accepted slice cannot be orphaned.
