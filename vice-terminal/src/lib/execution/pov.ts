...
async function tick(id: string): Promise<void> {
	...
	if (job.currentOrderId) {
		...
		if (filled <= 0) { save(transitionLocalAlgoJob(job, 'paused', 'POV child disappeared without an authoritative fill')); stop(id); return; }
	...
	if (job.deadmanMs) { const armed = await localExecution.armDeadman(job.deadmanMs); if (!armed.accepted) { save(transitionLocalAlgoJob(job, 'failed', armed.error ?? 'Dead-man switch could not be armed before POV child')); stop(id); return; } }
	...
	save(next);
	if (next.deadmanMs) { const armed = await localExecution.armDeadman(next.deadmanMs); if (!armed.accepted) { save(transitionLocalAlgoJob(next, 'failed', armed.error ?? 'Dead-man switch could not be re-armed')); stop(id); } }
}

function startTimer(id: string): void { if (timers.has(id)) return; timers.set(id, setInterval(() => void tickGuard.run(id, () => tick(id)).catch((error) => { const job = loadLocalAlgoJobs('pov').find((candidate) => candidate.id === id); if (job) save(transitionLocalAlgoJob(job, 'failed', error instanceof Error ? error.message : 'POV tick failed')); stop(id); }), 250)); void tickGuard.run(id, () => tick(id)); }
