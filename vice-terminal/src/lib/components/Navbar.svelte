...
<nav class="h-11 bg-terminal-bg border-b border-terminal-border flex items-center justify-between px-3 select-none gap-2">
	...
	<div class="flex items-center gap-1.5 px-2 py-1.5 bg-terminal-bg-secondary rounded border border-terminal-border text-xs min-w-0">
		...
		<span class="font-mono font-medium tabular-nums truncate">
			{#if $privacyMode}
				••••••
			{:else if canPresentAccountState($isConnected, $accountSyncStatus)}
	...
	<div class="flex items-center gap-1.5 flex-shrink-0">
		<span class="hidden sm:inline px-1.5 py-0.5 rounded text-3xs font-semibold uppercase {hyperliquidNetwork.isTestnet ? 'bg-terminal-yellow/15 text-terminal-yellow' : 'bg-terminal-red/20 text-terminal-red'}">{hyperliquidNetwork.network}</span>
		...
		<div class="hidden xl:block relative">
			...
			{#if panelMenuOpen}
				<div class="absolute right-0 top-full z-50 mt-1 w-32 rounded border border-terminal-border bg-terminal-bg-panel p-1 shadow-xl">
					...
					<button class="w-full rounded px-2 py-1 text-left text-3xs text-terminal-yellow hover:bg-terminal-bg-hover" onclick={() => { requestWorkspaceLayoutReset(); panelMenuOpen = false; }}>Reset this layout</button>
				</div>
			{/if}
		</div>
		<div class="hidden xl:block relative">
		...
		<button
			class="hidden lg:flex p-1.5 rounded text-terminal-text-secondary hover:text-terminal-text hover:bg-terminal-bg-hover"
			onclick={() => downloadLatencyEvidence(hyperliquidNetwork.network)}
		...
		<div class="hidden md:flex items-center gap-2 px-1 text-3xs text-terminal-text-muted">
			...
			{#if $isConnected && $activeAssetSyncStatus !== 'idle'}
