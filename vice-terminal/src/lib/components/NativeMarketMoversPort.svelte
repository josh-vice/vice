		...
		<div class="overflow-hidden rounded border border-terminal-border bg-terminal-bg-panel">
			<div class="flex flex-wrap items-center gap-2 border-b border-terminal-border px-3 py-2">
				<div class="flex rounded border border-terminal-border p-0.5 text-2xs">
					<button aria-pressed={direction === 'gainers'} class="rounded px-2 py-1 {direction === 'gainers' ? 'bg-terminal-green-bg text-terminal-green' : 'text-terminal-text-muted hover:text-terminal-text'}" onclick={() => (direction = 'gainers')}>Gainers</button>
					<button aria-pressed={direction === 'losers'} class="rounded px-2 py-1 {direction === 'losers' ? 'bg-terminal-red-bg text-terminal-red' : 'text-terminal-text-muted hover:text-terminal-text'}" onclick={() => (direction = 'losers')}>Losers</button>
				</div>
				<label class="ml-auto flex items-center gap-1.5 text-3xs text-terminal-text-muted">Rows
					<select aria-label="Mover rows" bind:value={limit} class="rounded border border-terminal-border bg-terminal-bg px-1 py-1 text-2xs text-terminal-text">
			...
			{:else}
				<div class="divide-y divide-terminal-border">
					{#each rows as market (market.marketKey)}
						<button class="grid w-full grid-cols-[minmax(0,1fr)_7rem_5.5rem_6rem] items-center px-3 py-2 text-left hover:bg-terminal-bg-hover focus-visible:bg-terminal-bg-hover" onclick={() => selectMarket(market)} title={`Select ${market.symbol} in Trade`}>
