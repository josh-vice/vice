<script lang="ts">
	...
	<div class="p-2 border-b border-terminal-border">
		<div class="flex items-center gap-1.5 px-2 py-1 bg-terminal-bg rounded border border-terminal-border focus-within:border-terminal-cyan/50">
			...
			<input data-action-id="ui.src.lib.components.marketwatchlist.input.h5027c1a4e5" id="market-search" type="text" placeholder="Search market, class, category, or DEX" bind:value={$searchQuery} class="flex-1 bg-transparent text-2xs outline-none min-w-0" />
		</div>
		<div class="mt-1.5 flex gap-1">
			<select data-action-id="ui.src.lib.components.marketwatchlist.select.hbc432d3f57" aria-label="Price alert direction" bind:value={alertDirection} class="w-[72px] bg-terminal-bg rounded border border-terminal-border px-1 text-3xs">
				<option value="above">Above</option>
				<option value="below">Below</option>
			</select>
			<input data-action-id="ui.src.lib.components.marketwatchlist.input.h28b6af13cc" aria-label="Price alert threshold" type="number" min="0" step="any" placeholder={$selectedMarket ? `Alert ${$selectedMarket.symbol}` : 'Select a market'} bind:value={alertPrice} class="min-w-0 flex-1 bg-terminal-bg rounded border border-terminal-border px-2 text-3xs outline-none focus:border-terminal-cyan/50" />
			<button data-action-id="ui.src.lib.components.marketwatchlist.button.h032e4f8e5e" class="flex items-center gap-1 rounded border border-terminal-border px-1.5 text-3xs text-terminal-text-secondary hover:text-terminal-cyan disabled:opacity-50" onclick={addAlert} disabled={!$selectedMarket} title="Create local price alert"><Bell class="w-3 h-3" />Add</button>
		</div>
		...
		{#if $priceAlertNotice}
			<div class="mt-1 flex items-center gap-1 rounded border border-terminal-cyan/40 bg-terminal-cyan/10 px-1.5 py-1 text-3xs text-terminal-cyan" role="status">
				<Bell class="w-3 h-3" /><span class="min-w-0 flex-1 truncate">{$priceAlertNotice.message}</span><button data-action-id="ui.src.lib.components.marketwatchlist.button.h9a7f675f05" aria-label="Dismiss price alert" onclick={clearPriceAlertNotice}><X class="w-3 h-3" /></button>
			</div>
		{/if}
	</div>
	...
	<div data-action-id="market.watchlist.scroll" class="flex-1 overflow-y-auto scrollbar-none" bind:clientHeight={viewportHeight} onscroll={(event) => (scrollTop = event.currentTarget.scrollTop)}>
		{#if rows.length === 0}
		...
		{:else}
			...
			{#each virtualRows as row (row.key)}
				{#if row.kind === 'header'}
					<button data-action-id="ui.src.lib.components.marketwatchlist.button.hb7409506bd" aria-expanded={!collapsed.has(row.group.id)} style="height:{ROW_HEIGHT}px" class="w-full px-2 flex items-center gap-1 bg-terminal-bg-secondary border-b border-terminal-border text-3xs uppercase tracking-wide text-terminal-text-secondary" onclick={() => toggleGroup(row.group.id)}>
	...
	{#if $priceAlerts.length > 0}
		<div class="max-h-16 overflow-y-auto border-t border-terminal-border px-2 py-1 text-3xs text-terminal-text-muted">
			{#each $priceAlerts as alert (alert.id)}
				<div class="flex items-center gap-1"><Bell class="w-3 h-3 text-terminal-cyan" /><span class="min-w-0 flex-1 truncate">{alert.symbol} {alert.direction} {alert.threshold}</span><button data-action-id="ui.src.lib.components.marketwatchlist.button.hc22b88a046" aria-label={`Remove ${alert.symbol} alert`} onclick={() => removePriceAlert(alert.id)} class="hover:text-terminal-red"><X class="w-3 h-3" /></button></div>
			{/each}
		</div>
	{/if}
</div>
