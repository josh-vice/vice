...
<main data-testid="native-notes-port" class="min-h-screen bg-terminal-bg px-4 py-5 text-terminal-text sm:px-6">
	...
	<section class="mx-auto grid max-w-6xl gap-4 py-5 lg:grid-cols-[14rem_minmax(0,1fr)_18rem]">
		<aside class="rounded border border-terminal-border bg-terminal-bg-panel p-3">
			<div class="mb-3 flex items-center justify-between"><h2 class="text-xs font-semibold uppercase tracking-[0.16em]">Scratchpads</h2><button class="rounded border border-terminal-border px-2 py-1 text-2xs text-terminal-cyan hover:border-terminal-cyan" onclick={addNote}>New</button></div>
			<div class="space-y-1">
				{#each $nativeNotes as note (note.id)}
					<button class="w-full truncate rounded px-2 py-2 text-left text-xs {note.id === selected?.id ? 'bg-terminal-cyan/10 text-terminal-cyan' : 'text-terminal-text-secondary hover:bg-terminal-bg-hover'}" onclick={() => select(note)}>{note.title || 'Untitled scratchpad'}</button>
				{:else}
		...
		<section class="min-h-[32rem] rounded border border-terminal-border bg-terminal-bg-panel p-4">
			{#if selected}
				<div class="mb-3 flex gap-2"><input aria-label="Scratchpad title" class="min-w-0 flex-1 bg-transparent text-lg font-semibold outline-none placeholder:text-terminal-text-muted" bind:value={draftTitle} oninput={queueSave} placeholder="Untitled scratchpad" /><button class="rounded border border-terminal-border px-2 py-1 text-2xs text-terminal-red hover:border-terminal-red" onclick={deleteSelected}>Delete</button></div>
				<textarea aria-label="Scratchpad text" class="min-h-[26rem] w-full resize-y bg-transparent font-mono text-sm leading-6 outline-none placeholder:text-terminal-text-muted" bind:value={draftText} oninput={queueSave} placeholder="Write your trade plan, levels, and reminders…"></textarea>
			{:else}
				<div class="flex min-h-[26rem] items-center justify-center text-center text-sm text-terminal-text-muted"><div><p>No scratchpad selected.</p><button class="mt-3 rounded border border-terminal-border px-3 py-2 text-xs text-terminal-cyan hover:border-terminal-cyan" onclick={addNote}>Create scratchpad</button></div></div>
			{/if}
		</section>
