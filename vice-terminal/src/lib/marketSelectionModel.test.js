...
const markets = [
	...
	{ kind: 'outcome', marketKey: 'outcome:1:0', apiCoin: '#1', symbol: 'Yes', name: 'Outcome', baseToken: 'Yes', quoteToken: 'USDC', dex: null }
];
...
describe('market selection model', () => {
	test('matches product classes without an option fallthrough', () => {
		...
		expect(markets.filter((market) => marketMatchesType(market, 'prediction'))).toHaveLength(1);
	});
