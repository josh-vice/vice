...
describe('US-003 chart interaction safety', () => {
	test('keeps context-menu submission explicitly armed and restores drag pending state when identity is missing', async () => {
		...
		expect(source).toContain("candidate.id === order.id ? { ...candidate, pending: false, error }");
		expect(source).toContain('crossSpreadWarning = warning ??');
		expect(source).toContain('liveOrderStatusLabel(order)');
		...
		expect(source).toContain('style={`top:${Math.max(0, y - 11)}px`}');
		expect(source).toContain('priceFromClick(chart, candlestickSeries, param, $selectedMarket)');
		expect(source).toContain('chartPriceMinMove($selectedMarket, previousPrice)');
		expect(source).toContain('data-testid="chart-designer-draft"');
		expect(source).toContain('pointer-events-none transition-[top] duration-150');
		expect(source).toContain('↕ DRAG');
		expect(source).toContain('pointer-events-auto cursor-ns-resize');
		expect(source).toContain('onpointerdown={startDesignerDrag}');
		expect(source).toContain('onpointermove={onDesignerPointerMove}');
		expect(source).toContain("event.key === 'ArrowUp' || event.key === 'ArrowRight'");
		expect(source).toContain("event.key === 'Escape'");
		expect(source).toContain('chartPriceMinMove($selectedMarket, designerPrice)');
		expect(source).toContain('designerDraftView.action');
		expect(source).toContain('onclick={toggleDesignerMode}');
