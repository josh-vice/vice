	...
	<div class="mx-auto grid max-w-[1120px] gap-12 xl:grid-cols-[minmax(0,760px)_190px]">
		<article class="min-w-0">
			<nav class="flex items-center gap-1.5 text-xs text-[#77798e]" aria-label="Breadcrumb">
				<a data-action-id="ui.src.routes.docs.slug.page.a.h1880386d95" href="/docs" class="hover:text-white">Docs</a><ChevronRight size={12} /><span>{topic.group}</span><ChevronRight size={12} /><span class="text-[#b9bac8]">{topic.label}</span>
			</nav>
			...
			{#if topic.visual}
				<figure class="mt-10 overflow-hidden rounded-md border border-[#272235] bg-[#0d0b18] p-3">
					...
					<a data-action-id="ui.src.routes.docs.slug.page.a.h03fec3cffd" href={topic.visual.src} target="_blank" rel="noreferrer" aria-label="Open {topic.label} screenshot at full size">
			...
			<div class="mt-12 divide-y divide-white/[0.09] border-y border-white/[0.09]">
				{#each topic.sections as section}
					<section id={section.id} class="scroll-mt-24 py-10">
						<a data-action-id="ui.src.routes.docs.slug.page.a.h2bbe846d8b" href="#{section.id}" class="group flex items-center gap-2">
		...
		<aside class="hidden xl:block">
			<div class="sticky top-24 border-l border-white/[0.09] pl-5">
				...
				<nav class="mt-3 space-y-1">{#each topic.sections as section}<a data-action-id="ui.src.routes.docs.slug.page.a.h7a4e32a023" href="#{section.id}" class="block py-1 text-xs leading-5 text-[#8b85a3] transition hover:text-[#00d4d4]">{section.title}</a>{/each}</nav>
			</div>
		</aside>
	</div>
</main>
