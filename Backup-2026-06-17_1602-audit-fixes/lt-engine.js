...
function _applyRsiPanel(opt, def, ctx) {
  ...
  opt.series.push({
    ...
    markLine: {
      ...
      data: [
        { yAxis: 70, label: { show: true, formatter: '70', position: 'start', color: C.TEXT3, fontSize: 9 }, lineStyle: { color: 'rgba(204,68,68,0.5)', type: 'dashed', width: 1 } },
        { yAxis: 30, label: { show: true, formatter: '30', position: 'start', color: C.TEXT3, fontSize: 9 }, lineStyle: { color: 'rgba(40,200,120,0.5)', type: 'dashed', width: 1 } }
      ]
    }
  });
...
function _applyLiqCluster(opt, def) {
  ...
  // dark text on light pills (gold/teal), white text on dark pills (red) — keep it legible
  const _p = rgb.split(',').map(Number), _lum = (_p[0] * 0.299 + _p[1] * 0.587 + _p[2] * 0.114);
  const labelTxt = _lum > 140 ? '#1a1208' : '#fff';   // dark on gold/teal, white on red
...
// Trading-activity sub-panel: cumulative delta in its own grid below price — RED below
// zero (sellers aggressive), teal above (buyers), with a zero line. Conveys "very red
// delta → flips on the squeeze."  def.tradingActivity = true (+ optional def.activityLabel)
function _applyActivityPanel(opt, def, ctx) {
  const ohlc = (ctx && ctx.ohlc) || def.ohlc || [];
  if (!ohlc.length) return;
  const C = chartColors(), delta = _computeCumDelta(ohlc), g0 = opt.grid || {};
  opt.grid = [
    Object.assign({}, g0, { bottom: '30%' }),
  ...
  // NB: a sign-coloured histogram (bar), NOT a line — piecewise visualMap on a `line`
  // series throws in this ECharts build ("reading 'coord'"); bars colour cleanly.
  opt.series.push({
    name: def.activityLabel || 'Cumulative Delta', type: 'bar', data: delta, xAxisIndex: 1, yAxisIndex: 1,
    barWidth: '60%', silent: true, z: 2,
    markLine: { symbol: ['none', 'none'], silent: true, data: [
      { yAxis: 0, label: { show: true, formatter: def.activityLabel || 'Cum Δ', position: 'insideStartTop', color: C.TEXT3, fontSize: 9, fontFamily: 'JetBrains Mono, monospace' }, lineStyle: { color: C.BORDER, type: 'dashed', width: 1 } }
    ] }
  });
  // colour by sign: red = sellers aggressive (delta < 0), teal = buyers aggressive (≥ 0)
  opt.visualMap = (opt.visualMap || []).concat([{
    show: false, type: 'piecewise', seriesIndex: si, dimension: 1,
    pieces: [{ lt: 0, color: '#cc2222' }, { gte: 0, color: '#00d4d4' }]
  }]);
  if (opt.tooltip) opt.tooltip.axisPointer = Object.assign({}, opt.tooltip.axisPointer, { link: [{ xAxisIndex: 'all' }] });
}
...
function _applySubPanel(opt, def, ctx) {
  ...
  if (!n) return;
  const C = chartColors(), data = _interpSeries(sp.points, n), g0 = opt.grid || {};
  opt.grid = [
    Object.assign({}, g0, { bottom: '30%' }),
    { left: g0.left, right: g0.right, top: '78%', bottom: 26 }
  ];
  ...
  if (sp.kind === 'oi' || sp.type === 'line') {
    const col = sp.color || '255,190,40';   // OI: amber line, no sign meaning (always +)
    opt.series.push({
    ...
    opt.series.push({
      ...
      markLine: { symbol: ['none', 'none'], silent: true, data: [
        { yAxis: 0, label: { show: true, formatter: label, position: 'insideStartTop', color: C.TEXT3, fontSize: 9, fontFamily: 'JetBrains Mono, monospace' }, lineStyle: { color: C.BORDER, type: 'dashed', width: 1 } }
      ] }
    });
    opt.visualMap = (opt.visualMap || []).concat([{
      show: false, type: 'piecewise', seriesIndex: si, dimension: 1,
      pieces: [{ lt: 0, color: sp.negColor || '#cc2222' }, { gte: 0, color: sp.posColor || '#28c878' }]
    }]);
  }
  if (opt.tooltip) opt.tooltip.axisPointer = Object.assign({}, opt.tooltip.axisPointer, { link: [{ xAxisIndex: 'all' }] });
}
...
function _applyOrderBook(opt, def) {
  ...
  side(ob.bids, ob.bidColor || '40,200,120', ob.bidLabel || 'Bid Wall (support)');
  side(ob.asks, ob.askColor || '204,46,46', ob.askLabel || 'Ask Wall (resistance)');
}
...
// Lead-in: a small consolidation/approach whose LAST close == the chart's first open.
function _genApproach(endOpen, n, range, seed) {
  if (typeof ltCandles !== 'function') return [];
  const amp = Math.max(range * 0.10, Math.abs(endOpen) * 0.01);
  const h = Math.ceil(n / 2), k = n - h;
  return ltCandles(endOpen - amp * 0.5, [{ to: endOpen + amp * 0.5, bars: h }, { to: endOpen, bars: k }], { seed: seed, wick: 0.65, noise: 0.7 });
}
// Follow-through: a gentle drift/settle out of the chart's last close.
function _genTrail(startClose, n, range, seed) {
  if (typeof ltCandles !== 'function') return [];
  const amp = Math.max(range * 0.08, Math.abs(startClose) * 0.008);
  const h = Math.ceil(n / 2), k = n - h;
  return ltCandles(startClose, [{ to: startClose + amp * 0.45, bars: h }, { to: startClose - amp * 0.15, bars: k }], { seed: seed, wick: 0.65, noise: 0.7 });
}
...
function _expandChartDef(def) {
  ...
  const lead = _genApproach(ohlc[0][0], _CTX_LEAD, range, seed);
  const trail = _genTrail(ohlc[ohlc.length - 1][1], _CTX_TRAIL, range, seed + 101);
...
function _deconflictMarkLabels(elId) {
  ...
  meas.font = '700 10px "JetBrains Mono", monospace';
  const GAP = 4;
  const boxes = data.map(m => {
    ...
    const w = meas.measureText(text).width + 16;
    const h = 19;
