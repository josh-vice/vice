...
function _subPanelGrids(opt, opts) {
  ...
  opt.yAxis = [
    ...
    { gridIndex: 1, scale: opts.min == null, min: opts.min, max: opts.max,
      ...
      axisLabel: opts.fmt ? { show: true, color: C.TEXT3, fontSize: 8.5, fontFamily: 'JetBrains Mono, monospace', formatter: opts.fmt } : { show: false } }
  ];
  if (opt.tooltip) opt.tooltip.axisPointer = Object.assign({}, opt.tooltip.axisPointer, { link: [{ xAxisIndex: 'all' }] });
  return C;
}
...
function _applyActivityPanel(opt, def, ctx) {
  ...
  opt.series.push({
    ...
    symbol: 'none', showSymbol: false, silent: true, smooth: false, lineStyle: { color: '#ffbe28', width: 2 }, z: 3,
...
function _applyVolumePanel(opt, def, ctx) {
  ...
  const data = ohlc.map((c, i) => ({ value: vols[i], itemStyle: { color: c[1] >= c[0] ? 'rgba(0,212,212,0.5)' : 'rgba(204,46,46,0.5)' } }));
...
function _applySubPanel(opt, def, ctx) {
  ...
  const hint = sp.kind === 'funding' ? '  · − = shorts pay (squeeze fuel)'
