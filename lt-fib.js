...
(function () {
  ...
  var STEP_MS = 2500;   // dwell per phase — long enough to read each caption
  ...
  function _svg() {
    var fibLines = LEVELS.map(function (L) {
      ...
      return '<line x1="70" y1="' + y + '" x2="326" y2="' + y + '" stroke="' + (gold ? G : S) + '" stroke-width="' + (gold ? 1.3 : 1) + '" stroke-dasharray="4 3" opacity="' + (gold ? 0.9 : 0.5) + '"/>'
        + '<text x="64" y="' + (y + 3) + '" text-anchor="end" fill="' + (gold ? G : S) + '" font-size="8.5" font-family="Geist Mono,monospace" font-weight="700">' + L.lab + '</text>';
    }).join('');
    var pocket = '<rect x="70" y="' + _y(0.618) + '" width="256" height="' + (_y(0.66) - _y(0.618)) + '" fill="rgba(231,181,58,0.22)"/>'
      + '<text x="322" y="' + (_y(0.618) - 3) + '" text-anchor="end" fill="' + G + '" font-size="8.5" font-family="Geist Mono,monospace" font-weight="700">golden pocket</text>';
  ...
  function _fibStyles() {
      ...
      ".fib-wrap{width:100%;max-width:460px;margin:0 auto;font-family:'Geist Mono',ui-monospace,monospace;}",
