...
function _applyRsiPanel(opt, def, ctx) {
  ...
  opt.xAxis = [
    ...
    Object.assign({}, baseX, { gridIndex: 1, data: xData,
      ...
      axisLabel: { color: C.TEXT3, fontSize: 9, fontFamily: 'Geist Mono, monospace', hideOverlap: true } })
  ];
  ...
  opt.yAxis = [
    ...
    { gridIndex: 1, min: 0, max: 100, interval: 50, scale: false,
      ...
      axisLabel: { color: C.TEXT3, fontSize: 9, fontFamily: 'Geist Mono, monospace' } }
  ];
...
function _applyLiqCluster(opt, def) {
  ...
  lc.lines.forEach((y, i) => {
    ...
    s0.markLine.data.push([
      { yAxis: y,
        label: labelled ? { show: true, formatter: lc.label, color: labelTxt, fontSize: 9, fontWeight: 700, fontFamily: 'Geist Mono, monospace', position: 'end', backgroundColor: col, padding: [2, 6], borderRadius: 4 } : { show: false },
...
function _applyHeatmap(opt, def) {
  ...
  def.heatmap.forEach(b => {
    ...
    if (b.label) s0.markArea.data.push([
      { yAxis: bestY0, label: { show: true, formatter: b.label, color: '#fff', fontSize: 9, fontWeight: 700, fontFamily: 'Geist Mono, monospace', position: 'insideTopLeft', backgroundColor: 'rgba(10,10,12,0.5)', padding: [2, 5], borderRadius: 3 }, itemStyle: { color: 'transparent' } },
      { yAxis: bestY0 + cellH }
    ]);
  });
}
...
function _subPanelGrids(opt, opts) {
  ...
  opt.xAxis = [
    Object.assign({}, baseX, { gridIndex: 0, axisLabel: { show: false }, axisTick: { show: false } }),
    Object.assign({}, baseX, { gridIndex: 1, data: xData, axisLine: { lineStyle: { color: C.BORDER } }, axisTick: { show: false }, axisLabel: { color: C.TEXT3, fontSize: 9, fontFamily: 'Geist Mono, monospace', hideOverlap: true } })
  ];
  const baseY = opt.yAxis || {};
  opt.yAxis = [
    Object.assign({}, baseY, { gridIndex: 0 }),
    { gridIndex: 1, scale: opts.min == null, min: opts.min, max: opts.max,
      ...
      axisLabel: opts.fmt ? { show: true, color: C.TEXT2, fontSize: 9, fontFamily: 'Geist Mono, monospace', formatter: opts.fmt } : { show: false } }
  ];
  if (opt.tooltip) opt.tooltip.axisPointer = Object.assign({}, opt.tooltip.axisPointer, { link: [{ xAxisIndex: 'all' }] });
  return C;
}
...
function _applyActivityPanel(opt, def, ctx) {
  ...
  opt.series.push({
    ...
    markLine: { symbol: ['none', 'none'], silent: true, data: [
      { yAxis: 0, label: { show: true, formatter: (def.activityLabel || 'Cumulative Δ (CVD)'), position: 'insideStartTop', color: C.TEXT3, fontSize: 9, fontFamily: 'Geist Mono, monospace' }, lineStyle: { color: C.BORDER, type: 'dashed', width: 1 } }
    ] }
  });
}
...
function _applyVolumePanel(opt, def, ctx) {
  ...
  opt.series.push({
    ...
    markLine: { symbol: ['none', 'none'], silent: true, data: [
      { yAxis: +avg.toFixed(2), label: { show: true, formatter: (cfg.label || 'Volume') + ' · avg', position: 'insideStartTop', color: C.TEXT3, fontSize: 9, fontFamily: 'Geist Mono, monospace' }, lineStyle: { color: C.BORDER, type: 'dashed', width: 1, opacity: 0.6 } }
    ] }
  });
}
...
function _applySubPanel(opt, def, ctx) {
  ...
  if (sp.kind === 'oi' || sp.type === 'line') {
    ...
    opt.series.push({
      ...
      markLine: { symbol: ['none', 'none'], silent: true, data: [
        { yAxis: data[0], label: { show: true, formatter: label, position: 'insideStartTop', color: C.TEXT3, fontSize: 9, fontFamily: 'Geist Mono, monospace' }, lineStyle: { color: C.BORDER, type: 'dashed', width: 1, opacity: 0.5 } }
      ] }
    });
  } else {
    opt.series.push({
      ...
      markLine: { symbol: ['none', 'none'], silent: true, data: [
        { yAxis: 0, label: { show: true, formatter: label + hint, position: 'insideStartTop', color: C.TEXT3, fontSize: 9, fontFamily: 'Geist Mono, monospace' }, lineStyle: { color: C.BORDER, type: 'dashed', width: 1 } }
      ] }
    });
    opt.visualMap = (opt.visualMap || []).concat([{
      show: false, type: 'piecewise', seriesIndex: si, dimension: 1,
...
function _applyMultiSubPanels(opt, def, ctx) {
  ...
  for (let k = 0; k < N; k++) {
    ...
    xAxes.push(Object.assign({}, baseX, {
      ...
      axisLabel: isLast ? { color: C.TEXT3, fontSize: 9, fontFamily: 'Geist Mono, monospace', hideOverlap: true } : { show: false }
    }));
  ...
  panels.forEach((sp, k) => {
    ...
    const nameMarkLine = (yv, extra) => ({ symbol: ['none', 'none'], silent: true, data: [
      { yAxis: yv, label: { show: true, formatter: label + (extra || ''), position: 'insideStartTop', color: C.TEXT3, fontSize: 9, fontFamily: 'Geist Mono, monospace' }, lineStyle: { color: C.BORDER, type: 'dashed', width: 1, opacity: 0.5 } }
    ] });
    const si = opt.series.length;
...
function _applyOrderBook(opt, def) {
  ...
  function side(rungs, rgb, label) {
    ...
    rungs.forEach(r => {
      ...
      s0.markLine.data.push([
        { yAxis: r.price,
          label: (isWall && label) ? { show: true, formatter: label, color: '#0a0a0c', backgroundColor: 'rgba(' + rgb + ',0.95)', fontSize: 9, fontWeight: 700, fontFamily: 'Geist Mono, monospace', padding: [2, 6], borderRadius: 4, position: 'end' } : { show: false },
...
function buildCandlestickOption(def, revealMode) {
    ...
    revealDivider.push([
      { xAxis: _dividerX,
        label: { show: true, formatter: 'Decision', rotate: 0, position: 'insideEndTop',
                 color: '#0b0b0e', fontSize: sm ? 8 : 9, fontWeight: 700, fontFamily: 'Geist Mono, monospace',
    ...
    futureArea = [
      { xAxis: _dividerX, y: '0%', itemStyle: { color: 'rgba(148,163,184,0.06)' },
        label: { show: !sm, formatter: 'Revealed', position: 'insideTopRight', distance: 8,
                 color: 'rgba(203,213,225,0.5)', fontSize: 9, fontWeight: 700, fontFamily: 'Geist Mono, monospace' } },
      { x: '100%', y: '100%' }
    ];
  }
  ...
  const _opt = {
    ...
    tooltip: {
      ...
      backgroundColor: C.BG4,
      borderColor: C.BORDER,
      textStyle: { color: C.TOOLTIP_TEXT, fontSize: 11, fontFamily: 'Geist Mono, monospace' },
      formatter(params) {
        let html = '';
    ...
    xAxis: {
      ...
      axisTick:  { show: false },
      axisLabel: { color: C.TEXT3, fontSize: sm ? 9 : 10, fontFamily: 'Geist Mono, monospace', hideOverlap: true, interval: _timeAxis.interval },
    ...
    yAxis: {
      ...
      axisLabel: { color: C.TEXT2, fontSize: sm ? 9 : 10, fontFamily: 'Geist Mono, monospace', formatter: (val) => _engRoundTick(val) }
    },
    series: [{
      name:  'Price',
      type:  'candlestick',
      ...
      markPoint: {
        symbol: 'pin',
        symbolSize: 16,
        label: { fontFamily: 'Geist Mono, monospace' },
        data: mpData
      },
      markLine: {
        symbol: ['none','none'],
...
function buildLineOption(def) {
  ...
  return {
    ...
    legend: hasLegend ? {
      ...
      textStyle: { color: C.TEXT2, fontSize: 11, fontFamily: 'Geist Mono, monospace' },
      top: 4
    } : undefined,
    tooltip: {
      trigger: 'axis',
    ...
    xAxis: {
      ...
      data: labels,
      axisLine:  { lineStyle: { color: C.BORDER } },
      axisLabel: { color: C.TEXT3, fontSize: _smLine ? 9 : 10, fontFamily: 'Geist Mono, monospace', hideOverlap: true },
      splitLine: { show: false }
    },
    yAxis: {
      scale:     true,
      ...
      axisLine:  { lineStyle: { color: C.BORDER } },
      axisLabel: { color: C.TEXT2, fontSize: _smLine ? 9 : 10, fontFamily: 'Geist Mono, monospace', formatter: (v) => (def.yPrefix || '') + _engRoundTick(v) + (def.ySuffix || '') }
    },
    series
  };
}
...
function _deconflictMarkLabels(elId) {
  ...
  meas.font = '700 ' + fs + 'px "Geist Mono", monospace';
  const GAP = 4;
...
function renderChart(elId, def, revealMode) {
  ...
  LTUtils.echartsZoomShim(el);   // correct hover coords under the laptop body{zoom}
...
function renderTeachingChart(elId, def) {
  ...
  const mlData = (def.markLines || []).map(ml => [{
    yAxis:     ml.yAxis,
    label:     { show: true, formatter: ml.label, color: _pillText(ml.color || TEAL), fontSize: 10, fontWeight: 700, fontFamily: 'Geist Mono, monospace', position: 'end', backgroundColor: ml.color || TEAL, padding: [2, 6], borderRadius: 4 },
    lineStyle: { color: ml.color || TEAL, type: 'dashed', width: 1.5, opacity: 0.8 }
  }, { yAxis: ml.yAxis }]);
  ...
  function runTeachAnimation() {
    ...
    const teachDivider = [
      [
        { xAxis: decisionIdx, label: { show: true, formatter: 'Decision', color: '#0b0b0e', fontSize: 10, fontWeight: 700, fontFamily: 'Geist Mono, monospace', position: 'insideEndTop', backgroundColor: TEAL, padding: [2, 6], borderRadius: 4 }, lineStyle: { color: TEAL, type: 'dashed', width: 1.5, opacity: 0.85 } },
        { xAxis: decisionIdx }
      ],
      ...mlData
    ];
...
function _riskCalloutHtml(chapter) {
  ...
  if (LTStore.get('riskDismissed') === '1') return '';   // user closed it — stay closed
  return `
    <div class="risk-callout" role="note">
      ...
      <button class="risk-callout-x" aria-label="Dismiss this warning" onclick="ltDismissRiskCallout(this)">
...
/* Dismiss the risk callout: remembered across sessions (it renders on every
   leverage/derivatives lesson otherwise). Collapses smoothly, instant under
   reduced motion. */
window.ltDismissRiskCallout = function (btn) {
  LTStore.set('riskDismissed', '1');
  const el = btn.closest('.risk-callout');
  if (!el) return;
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) { el.remove(); return; }
  el.style.height = el.offsetHeight + 'px';   // fix the height so it can transition to 0
  requestAnimationFrame(() => {
    el.classList.add('risk-callout--out');
...
const _DEMO_WIDGETS = {
  gallery:    { mount: (id) => { if (typeof renderCandlestickGallery === 'function') renderCandlestickGallery(id); } },
...
function updateHeaderUI() {
  ...
  _ltApplyTitleMarquee();
}

/* Ticker marquee for a truncated #header-title: only when the text actually
   overflows (scrollWidth > clientWidth) and motion is allowed. Resets to plain
   text first so the measurement is clean, then wraps the text in a span whose
   keyframed translateX (see .header-title--marquee in lt-styles.css) slowly
   reveals the tail, pauses, and slides back. */
function _ltApplyTitleMarquee() {
  const el = document.getElementById('header-title');
  if (!el) return;
  const text = el.textContent;
  el.classList.remove('header-title--marquee');
  el.textContent = text;                        // clean measurement state
  if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  const overflow = el.scrollWidth - el.clientWidth;
  if (overflow <= 4) return;                    // not meaningfully truncated
  const span = document.createElement('span');
  span.className = 'ht-marquee';
  span.textContent = text;
  // Slide covers 34% of the cycle each way → duration keeps the pace near a
  // slow ~20px/s regardless of how much text is hidden.
  const dur = Math.min(26, Math.max(10, (overflow / 20) / 0.34));
  span.style.setProperty('--ht-shift', `-${overflow}px`);
  span.style.setProperty('--ht-dur', `${dur.toFixed(2)}s`);
  el.textContent = '';
  el.appendChild(span);
  el.classList.add('header-title--marquee');
}
let _ltTitleMarqueeTimer = 0;
window.addEventListener('resize', () => {
  clearTimeout(_ltTitleMarqueeTimer);
  _ltTitleMarqueeTimer = setTimeout(_ltApplyTitleMarquee, 120);
});
// Geist Mono loads async — a title measured against the fallback font under-
// (or over-)estimates the overflow, so re-measure once real metrics are in.
if (document.fonts && document.fonts.ready) document.fonts.ready.then(() => _ltApplyTitleMarquee());
...
function scrollActiveChapterIntoView() {
  ...
  if (Math.abs(delta) > 1) {
    ...
    // delta is a gBCR (visual px) difference; scrollBy takes layout px — undo the body zoom
    const z = list.clientHeight ? (listRect.height / list.clientHeight) : 1;
    list.scrollBy({ top: delta / z, behavior: reduce ? 'auto' : 'smooth' });
  }
}
...
function _ltInitChapterContextMenu() {
  ...
  function openMenuFor(item, cx, cy) {
    ...
    // clientX/Y and innerWidth/Height are visual px, but the fixed menu lives
    // inside the zoomed body where left/top are layout px — divide by the zoom
    const z = document.body.offsetWidth ? (document.body.getBoundingClientRect().width / document.body.offsetWidth) : 1;
    let x = cx / z, y = cy / z;
    if (x + mw > window.innerWidth  / z - 8) x = window.innerWidth  / z - mw - 8;
    if (y + mh > window.innerHeight / z - 8) y = window.innerHeight / z - mh - 8;
    if (x < 8) x = 8;
    if (y < 8) y = 8;
...
function showBackupReminder() {
  ...
  if (!document.getElementById('lt-backup-pop-styles')) {
    ...
    st.textContent = `
      ...
      .lt-backup-pop-title { font-family:'Geist Mono',monospace; font-size:18px; font-weight:800; color:var(--text); margin-bottom:6px; }
      ...
      .lt-backup-pop-btn { font-family:'Geist Mono',monospace; font-size:12px; font-weight:700; padding:7px 13px; border-radius:var(--radius); cursor:pointer; transition:all .15s; }
