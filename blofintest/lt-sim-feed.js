...
/* ═══════════════════════════════════════════════════════════════════════════
   BloFin Academy STAGE copy (/blofintest) of lt-sim-feed.js
   Identical to prod except the provider chain is fronted by BloFin's public
   demo-trading market-data API (verified live + CORS-open 2026-07-27):
     REST  https://demo-trading-openapi.blofin.com/api/v1/market/…
     WS    wss://demo-trading-openapi.blofin.com/ws/public
   Candle rows come NEWEST-first as [ts,o,h,l,c,vol,volCcy,volQuote,confirm];
   volCcy (index 6) is the BTC-denominated volume, matching the other venues.
   If BloFin ever fails, the original chain below takes over automatically.
   ───────────────────────────────────────────────────────────────────────────
   Liquidity Theory — Practice Simulator: LIVE BTC market data feed
   lt-sim-feed.js

   Public, key-less market data with a provider fallback chain:
     BloFin Demo (BTC-USDT) → Coinbase Exchange (BTC-USD) → Kraken (XBT/USD) → Binance (BTCUSDT)
...
(function (g) {
  ...
  const _BF = 'https://demo-trading-openapi.blofin.com/api/v1/market';
  function _bfRows(rows, limit) {
    // rows NEWEST-first: [ts, o, h, l, c, vol, volCurrency, volQuote, confirm]
    rows = (limit ? rows.slice(0, limit) : rows).reverse();
    return {
  ...
  const PROVIDERS = [
    {
      key: 'blofin', name: 'BloFin Demo',
      klines(tfMin, limit) {
        const bar = { 1: '1m', 5: '5m', 15: '15m', 60: '1H' }[tfMin] || '15m';
        return _fetchJson(_BF + '/candles?instId=BTC-USDT&bar=' + bar + '&limit=' + Math.min(limit, 300))
          .then(j => {
            if (j.code !== '0' || !Array.isArray(j.data)) throw new Error('blofin ' + j.code);
            return _bfRows(j.data, limit);
          });
      },
      older(tfMin, beforeMs, limit) {
        const bar = { 1: '1m', 5: '5m', 15: '15m', 60: '1H' }[tfMin] || '15m';
        return _fetchJson(_BF + '/candles?instId=BTC-USDT&bar=' + bar + '&after=' + beforeMs + '&limit=' + Math.min(limit, 300))
          .then(j => {
            if (j.code !== '0' || !Array.isArray(j.data)) throw new Error('blofin ' + j.code);
            return _bfRows(j.data, 0);
          });
      },
      pollPx() {
        return _fetchJson(_BF + '/tickers?instId=BTC-USDT')
          .then(j => +((j.data && j.data[0] && j.data[0].last) || 0));
      },
      wsUrl: 'wss://demo-trading-openapi.blofin.com/ws/public',
      wsSubscribe(ws) { ws.send(JSON.stringify({ op: 'subscribe', args: [{ channel: 'trades', instId: 'BTC-USDT' }] })); },
      wsParse(msg, onTick) {
        if (msg && msg.arg && msg.arg.channel === 'trades' && Array.isArray(msg.data))
          for (const t of msg.data) onTick(+(t.price || t.px), +(t.size || t.sz) || 0);
      }
    },
    {
