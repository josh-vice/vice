...
const LT_GLOSSARY = [
  ...
  { id:'spinning-top', term:'Spinning Top', cat:'Candlesticks', aliases:['spinning top'], refs:[[1,0],[2,4]],
  ...
  { id:'shooting-star', term:'Shooting Star', cat:'Candlesticks', aliases:['shooting star'], refs:[[1,0],[2,4]],
  ...
  { id:'inverted-hammer', term:'Inverted Hammer', cat:'Candlesticks', aliases:['inverted hammer'], refs:[[1,0],[2,4]],
  ...
  { id:'marubozu', term:'Marubozu', cat:'Candlesticks', aliases:['marubozu'], refs:[[1,0],[2,4]],
  ...
  { id:'fvg', term:'Fair Value Gap (FVG)', cat:'Liquidity', aliases:['fair value gap','fvg','imbalance'], refs:[[4,3]],
  ...
  { id:'long-short', term:'Long / Short', cat:'Derivatives', aliases:['going long','going short','long position','short position'], refs:[[2,1],[3,4]],
  ...
  /* ── Patterns ── */
  { id:'bull-flag', term:'Bull Flag', cat:'Patterns', aliases:['bull flag'], refs:[[2,8]],
  ...
  { id:'trendline', term:'Trendline', cat:'Patterns', aliases:['trendline','trend line'], refs:[[1,3]],
  ...
  { id:'risk-reward', term:'Risk / Reward', cat:'Risk', aliases:['risk/reward','risk reward','reward to risk','r:r'], refs:[[1,10]],
  ...
  { id:'backtesting', term:'Backtesting', cat:'Risk', aliases:['backtest','backtesting'], refs:[[3,16]],
  ...
  { id:'long', term:'Long', cat:'Basics', aliases:['long','going long','long position','buy'], refs:[[2,1],[3,4]],
  ...
  { id:'short', term:'Short', cat:'Basics', aliases:['short','going short','short position','sell'], refs:[[2,1],[3,4]],
  ...
  { id:'divergence', term:'Divergence', cat:'Indicators', aliases:['divergence','bullish divergence','bearish divergence'],
    def:'When price and an oscillator (RSI, MACD) disagree — price posts a new high or low the oscillator does not confirm. A leading signal for a potential reversal (regular divergence) or continuation (hidden divergence).' },
  ...
  { id:'volatility', term:'Volatility (ATR)', cat:'Indicators', aliases:['volatility','atr','average true range'], refs:[[3,7]],
  ...
  { id:'head-shoulders', term:'Head & Shoulders', cat:'Patterns', aliases:['head and shoulders','head & shoulders','inverse head and shoulders'], refs:[[2,8]],
  ...
  { id:'spread', term:'Bid / Ask Spread', cat:'Derivatives', aliases:['spread','bid-ask','bid/ask','bid ask'], refs:[[3,3]],
  ...
  { id:'slippage', term:'Slippage', cat:'Derivatives', aliases:['slippage'], refs:[[3,5]],
  ...
  { id:'order-block', term:'Order Block', cat:'Liquidity', aliases:['order block','orderblock','ob','last up candle','last down candle'], refs:[[4,3]],
  ...
  { id:'cex', term:'Centralized Exchange (CEX)', cat:'Basics', aliases:['cex','centralized exchange','centralised exchange'], refs:[[3,2]],
  ...
  { id:'dex', term:'Decentralized Exchange (DEX)', cat:'Basics', aliases:['dex','decentralized exchange','decentralised exchange','amm'], refs:[[3,2]],
  ...
  { id:'cold-storage', term:'Cold Storage Wallet / Device', cat:'Basics', aliases:['cold storage','cold wallet','hardware wallet','ledger','trezor','offline wallet'], refs:[[3,2]],
...
function glFindReferences(entry) {
  ...
  const countIn = (text, t) => { const m = text.match(new RegExp('(^|[^a-z0-9])' + esc(t) + '([^a-z0-9]|$)', 'gi')); return m ? m.length : 0; };
  /* Relevance bar (owner-directed 2026-07-03): a reference must point at a chapter
     that TEACHES the term, not one that merely says the word. Keep a match only when
     the term appears in the chapter title/tag, OR ≥3 times in the chapter text, OR
     the entry pins it via entry.refs. Rank pins → title-backed → mention count, cap
     at 4 — a "point of direction" needs the best destinations, not all of them. */
  let scored = tokens.length ? cache.map(rec => {
    if (/recap|outro/i.test(rec.title || '')) return null;   // summary chapters teach nothing new
    const n = tokens.reduce((sum, t) => sum + countIn(rec.text, t), 0);
    if (!n) return null;
    const inTitle = tokens.some(t => countIn(((rec.title || '') + ' ' + (rec.tag || '')).toLowerCase(), t) > 0);
    return (inTitle || n >= 3) ? { rec, n, inTitle, pin: false } : null;
  }).filter(Boolean) : [];
  if (Array.isArray(entry.refs)) {
    entry.refs.forEach(pair => {
      const rec = cache.find(x => x.courseNum === pair[0] && x.chapterIdx === pair[1]);
      if (!rec) return;
      const hit = scored.find(x => x.rec === rec);
      if (hit) hit.pin = true;
      else scored.push({ rec, n: 0, inTitle: false, pin: true });
    });
  }
  scored.sort((a, b) => (b.pin - a.pin) || (b.inTitle - a.inTitle) || (b.n - a.n));
  const out = scored.slice(0, 4)
    .map(x => ({ courseNum: x.rec.courseNum, chapterIdx: x.rec.chapterIdx, title: x.rec.title, tag: x.rec.tag }))
    .sort((a, b) => a.courseNum - b.courseNum || a.chapterIdx - b.chapterIdx);
  _glRefCache[entry.id] = out;
  return out;
}
...
function _glStyles() {
  ...
  .gl-back-btn { display:inline-flex; align-items:center; gap:6px; background:none; border:none; color:var(--teal); font-family:'Geist Mono',monospace; font-size:13px; font-weight:600; cursor:pointer; padding:0; margin-bottom:12px; }
  .gl-back-btn:hover { opacity:.8; }
  .gl-heading { margin-bottom:14px; }
  .gl-title { font-family:'Geist Mono',monospace; font-size:21px; letter-spacing:-0.5px; font-weight:800; color:var(--text); margin:0; line-height:1.15; }
  ...
  .gl-search-box input { flex:1; background:none; border:none; outline:none; color:var(--text); font-family:'Geist Mono',monospace; font-size:14px; }
  ...
  .gl-cat-pill { display:inline-flex; align-items:center; gap:6px; background:var(--bg3); border:1px solid var(--border2); border-radius:100px; color:var(--text2); font-family:'Geist Mono',monospace; font-size:11.5px; font-weight:600; padding:5px 12px; cursor:pointer; transition:border-color .15s, color .15s, background .15s; }
  ...
  .gl-card { background:var(--bg3); border:1px solid var(--border); border-radius:var(--radius-lg); padding:16px 18px; margin-bottom:14px; scroll-margin-top:80px; }
  ...
  .gl-term { font-family:'Geist Mono',monospace; font-size:18px; letter-spacing:-0.3px; font-weight:800; color:var(--text); }
  ...
  .gl-figure { flex:0 0 188px; width:188px; padding:0; margin:0; border:none; background:none; cursor:zoom-in; display:block; font-family:inherit; -webkit-appearance:none; }
  ...
  .gl-lightbox-box { position:relative; width:100%; max-width:640px; max-height:calc(92vh / var(--app-zoom, 1)); overflow:auto; background:var(--bg3); border:1px solid var(--border2); border-radius:var(--radius-lg); padding:22px 24px 24px; box-shadow:0 32px 90px rgba(0,0,0,.7); animation:glPop .22s cubic-bezier(.34,1.56,.64,1); }
  ...
  .gl-lightbox-term { font-family:'Geist Mono',monospace; font-size:20px; font-weight:800; color:var(--text); }
  ...
  .gl-refs-toggle {
    ...
    border-radius:var(--radius); color:var(--text2); font-family:'Geist Mono',monospace; font-size:11px; font-weight:700;
...
function _gtx(x, y, s, col) { return `<text x="${x}" y="${y}" fill="${col || _GM}" font-size="8" font-family="Geist Mono,monospace" font-weight="700">${s}</text>`; }
// Flexible label: anchor (start|middle|end) + optional size.
function _glab(x, y, s, col, anchor, size) { return `<text x="${x}" y="${y}" fill="${col || _GM}" font-size="${size || 8}" font-family="Geist Mono,monospace" font-weight="700" text-anchor="${anchor || 'start'}">${s}</text>`; }
...
const _GC = '#a855f7';   // Chikou (lagging span) purple — matches the v2 lessons + engine overlay

/* ── Wide canvas (210×132) for the story figures — real candle-drawn mini-charts.
   The original 156×104 canvas stays for the single-candle anatomy figures. */
function _gsvg2(inner) { return `<svg viewBox="0 0 210 132" xmlns="http://www.w3.org/2000/svg" role="img" aria-hidden="true">${inner}</svg>`; }
function _gl2(y, col, dash, x1, x2) { return `<line x1="${x1 == null ? 8 : x1}" y1="${y}" x2="${x2 == null ? 202 : x2}" y2="${y}" stroke="${col}" stroke-width="1.1" ${dash ? 'stroke-dasharray="4 3"' : ''} opacity="0.8"/>`; }
function _run(cands) { return cands.map(c => _gc(c[0], c[1], c[2], c[3], c[4])).join(''); }

/* Full Ichimoku system for the component glossary figures: candles + a THICK
   two-tone Kumo with a visible bear→bull twist + forward projection, Kijun
   (gold), Tenkan (teal), Chikou (purple, plotted behind price). Pass a
   component key ('tenkan'|'kijun'|'chikou'|'cloud') to SPOTLIGHT it — that line
   goes bold and the rest dim — so the figure shows the WHOLE system yet points
   at the one the term names. Labels stay GENERIC (never the term) so the figure
   still works as a Definition→Term flashcard prompt. 'all' = whole system. */
function _ichiBase(hi) {
  const full = !hi || hi === 'all';
  const o = (k) => (full || hi === k) ? 1 : 0.22;
  const w = (k) => hi === k ? 2.6 : 1.5;
  const candles = _run([[28,96,84,80,98],[42,84,72,68,86],[56,72,78,68,82],[70,78,62,58,80],[84,62,50,46,64],[98,50,58,46,62],[112,58,44,40,60],[126,44,34,30,46],[140,34,26,22,36]]);
  const cloud = `<g opacity="${o('cloud')}">`
    + '<polygon points="8,86 40,84 64,77 40,80 8,78" fill="rgba(255,46,136,0.20)"/>'
    + '<polygon points="64,77 104,62 136,52 168,46 204,42 204,64 168,66 136,70 104,74" fill="rgba(40,200,120,0.20)"/>'
    + _gp('8,86 40,84 64,77 104,62 136,52 168,46 204,42', 'rgba(40,200,120,0.9)', w('cloud'))
    + _gp('8,78 40,80 64,77 104,74 136,70 168,66 204,64', 'rgba(255,46,136,0.7)', w('cloud'))
    + `</g>`;
  const kijun  = `<g opacity="${o('kijun')}">`  + _gp('36,86 70,78 104,68 140,58 176,50', _GG, w('kijun')) + '</g>';
  const tenkan = `<g opacity="${o('tenkan')}">` + _gp('36,82 56,74 70,70 84,60 98,58 112,52 126,44 140,38 156,34', _GT, w('tenkan')) + '</g>';
  const chikou = `<g opacity="${o('chikou')}">` + _gp('8,60 24,52 40,46 56,40 70,36', _GC, w('chikou')) + '</g>';
  const cg = `<g opacity="${full ? 1 : 0.45}">${candles}</g>`;
  const lab = hi === 'tenkan' ? _glab(204,28,'fast line',_GT,'end',7)
            : hi === 'kijun'  ? _glab(204,46,'slow line',_GG,'end',7)
            : hi === 'chikou' ? _glab(8,70,'plotted behind',_GC,'start',7)
            : hi === 'cloud'  ? _glab(200,78,'projected ahead',_GM,'end',7) + _glab(52,94,'twist',_GM,'middle',7)
            : '';
  return cloud + cg + kijun + tenkan + chikou + lab;
}
...
const GL_FIGURES = {
  /* — candle anatomy (original canvas, already candle-true) — */
  ...
  /* — patterns: candle-drawn stories (wide canvas) — */
  'bull-flag': _gsvg2(
    _run([[14,100,84,82,102],[28,84,66,64,86],[42,66,48,46,68],[56,48,36,32,50],[70,36,30,26,38]])
    + _gp('78,24 132,40', _GM, 1, true) + _gp('78,40 132,56', _GM, 1, true)
    + _run([[84,30,36,27,39],[98,36,41,33,44],[112,41,46,38,49],[126,46,43,40,50]])
    + _run([[140,44,26,22,46],[154,26,16,12,28],[168,16,10,6,18]])
    + _garr(178,12,198,6,_GT,1.6)
    + _glab(16,120,'impulse',_GT,'start',7) + _glab(105,124,'drift',_GM,'middle',7) + _glab(150,122,'continuation',_GT,'middle',7)),
  'bear-flag': _gsvg2(
    _run([[14,12,28,10,30],[28,28,46,26,48],[42,46,64,44,66],[56,64,76,62,78],[70,76,82,74,84]])
    + _gp('78,66 132,50', _GM, 1, true) + _gp('78,84 132,68', _GM, 1, true)
    + _run([[84,82,77,74,85],[98,77,72,69,79],[112,72,67,64,74],[126,67,70,63,73]])
    + _run([[140,72,88,70,90],[154,88,100,86,102],[168,100,110,98,112]])
    + _garr(178,112,198,118,_GR,1.6)
    + _glab(16,24,'impulse',_GR,'start',7) + _glab(105,40,'drift',_GM,'middle',7) + _glab(150,128,'continuation',_GR,'middle',7)),
  'double-top': _gsvg2(
    _gl2(24, _GR, true) + _gl2(64, _GM, true, 36)
    + _run([[14,96,80,78,98],[28,80,64,62,82],[42,64,46,44,66],[56,46,32,26,48],[70,32,52,30,54],[84,52,62,50,66],[98,62,44,42,64],[112,44,34,22,48],[126,34,50,32,52],[140,50,70,48,72],[154,70,84,68,86],[168,84,94,82,96]])
    + _glab(84,16,'equal highs',_GR,'middle',7) + _glab(198,58,'neckline',_GM,'end',7)),
  'double-bottom': _gsvg2(
    _gl2(98, _GT, true) + _gl2(56, _GM, true, 36)
    + _run([[14,24,40,22,42],[28,40,56,38,58],[42,56,74,54,76],[56,74,88,72,96],[70,88,68,66,90],[84,68,58,56,70],[98,58,76,56,78],[112,76,86,74,98],[126,86,70,68,88],[140,70,50,48,72],[154,50,36,34,52],[168,36,28,24,38]])
    + _glab(84,114,'equal lows',_GT,'middle',7) + _glab(198,50,'neckline',_GM,'end',7)),
  'head-shoulders': _gsvg2(
    _gl2(70, _GM, true)
    + _run([[14,90,74,72,92],[28,74,58,54,76],[42,58,72,56,74],[56,72,52,50,74],[70,52,32,26,54],[84,32,52,30,54],[98,52,66,50,68],[112,66,52,48,68],[126,52,68,50,70],[140,68,84,66,86],[154,84,94,82,96]])
    + _glab(198,64,'neckline',_GM,'end',7)),
  triangle: _gsvg2(
    _gp('8,28 160,58', _GM, 1.1, true) + _gp('8,96 160,64', _GM, 1.1, true)
    + _run([[14,84,48,42,90],[28,48,68,44,72],[42,68,54,48,74],[56,54,66,50,70],[70,66,58,52,72],[84,58,64,54,68],[98,64,60,56,68],[112,60,62,57,66]])
    + _run([[126,62,44,40,64],[140,44,32,28,46]])
    + _garr(148,34,170,22,_GT,1.6)),
  channel: _gsvg2(
    _gp('8,40 200,12', _GM, 1.1, true) + _gp('8,84 200,56', _GM, 1.1, true)
    + _run([[16,78,62,58,82],[30,62,50,46,64],[44,50,58,44,62],[58,58,44,40,60],[72,44,34,30,46],[86,34,44,32,48],[100,44,58,42,68],[114,58,46,42,60],[128,46,36,30,48],[142,36,44,34,50],[156,44,52,40,62],[170,52,40,36,54],[184,40,30,26,42]])),
  trend: _gsvg2(
    _run([[14,100,88,86,104],[28,88,76,72,90],[42,76,82,74,86],[56,82,64,60,84],[70,64,52,48,66],[84,52,60,50,64],[98,60,44,40,62],[112,44,34,30,46],[126,34,42,32,46],[140,42,28,24,44],[154,28,18,14,30],[168,18,24,16,28],[182,24,14,10,26]])
    + _gdot(70,48,_GR,2.4) + _gdot(112,30,_GR,2.4) + _gdot(154,14,_GR,2.4)
    + _gdot(84,64,_GT,2.4) + _gdot(126,46,_GT,2.4) + _gdot(168,28,_GT,2.4)
    + _glab(154,8,'HH',_GR,'middle',7) + _glab(126,58,'HL',_GT,'middle',7)),
  range: _gsvg2(
    _gl2(28, _GR, true) + _gl2(88, _GT, true)
    + _run([[14,84,44,40,88],[28,44,34,28,46],[42,34,54,30,58],[56,54,78,50,84],[70,78,86,74,88],[84,86,62,58,88],[98,62,34,30,66],[112,34,28,26,38],[126,28,52,26,56],[140,52,80,48,84],[154,80,84,76,88],[168,84,56,52,86],[182,56,36,28,58]])
    + _glab(10,20,'resistance',_GR,'start',7) + _glab(10,100,'support',_GT,'start',7)),
  breakout: _gsvg2(
    _gl2(44, _GR, true)
    + _run([[14,78,62,58,82],[28,62,70,58,74],[42,70,56,52,72],[56,56,64,50,66],[70,64,52,48,68],[84,52,58,46,62],[98,56,32,28,60],[112,32,24,20,36],[126,24,40,22,46],[140,40,42,36,48],[154,42,26,22,44],[168,26,16,12,28]])
    + _gdot(140,44,_GG,2.6)
    + _glab(10,38,'level',_GM,'start',7) + _glab(140,60,'retest',_GG,'middle',7)),
  'support-resistance': _gsvg2(
    _gl2(30, _GR, true) + _gl2(90, _GT, true)
    + _run([[14,86,66,62,90],[28,66,48,44,70],[42,48,42,28,52],[56,42,62,40,66],[70,62,84,60,92],[84,84,64,60,86],[98,64,40,36,66],[112,40,34,28,44],[126,34,56,32,60],[140,56,82,54,92],[154,82,60,56,84],[168,60,44,40,62]])
    + _glab(10,22,'resistance',_GR,'start',7) + _glab(10,102,'support',_GT,'start',7)),
  'supply-demand': _gsvg2(
    '<rect x="8" y="16" width="194" height="18" rx="2" fill="rgba(255,46,136,0.13)"/>'
    + '<rect x="8" y="82" width="194" height="18" rx="2" fill="rgba(0,212,212,0.13)"/>'
    + _run([[14,94,90,86,98],[28,90,70,66,92],[42,70,50,46,72],[56,50,58,46,62],[70,58,40,36,60],[84,40,26,22,42],[98,26,44,20,46],[112,44,62,42,66],[126,62,80,60,84],[140,80,88,76,96],[154,88,68,64,90],[168,68,54,50,70]])
    + _glab(12,28,'supply',_GR,'start',7) + _glab(12,96,'demand',_GT,'start',7)),
  pullback: _gsvg2(
    _run([[14,98,84,80,100],[28,84,68,64,86],[42,68,52,48,70],[56,52,40,36,54],[70,40,48,38,52],[84,48,54,44,58],[98,54,58,50,62],[112,58,44,40,60],[126,44,30,26,46],[140,30,20,16,32],[154,20,12,8,22]])
    + _glab(84,74,'temporary',_GM,'middle',7) + _glab(140,124,'trend resumes',_GT,'middle',7)),
  retest: _gsvg2(
    _gl2(56, _GT, true)
    + _run([[14,92,78,74,94],[28,78,64,60,80],[42,64,50,46,68],[56,50,40,36,52],[70,40,52,38,56],[84,52,58,50,62],[98,58,52,48,62],[112,52,38,34,54],[126,38,26,22,40],[140,26,18,14,28]])
    + _gdot(84,58,_GT,2.6)
    + _glab(10,48,'old ceiling',_GM,'start',7) + _glab(96,74,'holds',_GT,'middle',7)),
  swing: _gsvg2(
    _run([[14,84,66,62,88],[28,66,50,46,68],[42,50,38,30,52],[56,38,54,36,58],[70,54,66,52,72],[84,66,48,44,68],[98,48,32,22,50],[112,32,48,30,52],[126,48,58,46,64],[140,58,40,36,60],[154,40,28,24,42]])
    + _gdot(42,30,_GR,2.6) + _gdot(98,22,_GR,2.6) + _gdot(70,72,_GT,2.6) + _gdot(126,64,_GT,2.6)
    + _glab(98,14,'high',_GR,'middle',7) + _glab(70,84,'low',_GT,'middle',7)),
  ...
  ichimoku:      _gsvg2(_ichiBase('all')),   // full system: cloud + Tenkan + Kijun + Chikou over price
  fibonacci: _gsvg2(
    _run([[14,100,88,84,102],[28,88,72,68,90],[42,72,56,52,74],[56,56,40,36,58],[70,40,26,22,42],[84,26,16,10,28]])
    + _gl2(45, _GM, true, 92) + _gl2(56, _GM, true, 92) + _gl2(67, _GG, true, 92) + _gl2(82, _GM, true, 92)
    + _run([[98,16,34,14,38],[112,34,50,32,54],[126,50,62,48,68],[140,62,48,44,64],[154,48,34,30,50],[168,34,22,18,36]])
    + _glab(196,42,'.382',_GM,'end',7) + _glab(196,53,'.5',_GM,'end',7) + _glab(196,64,'.618',_GG,'end',7) + _glab(196,79,'.786',_GM,'end',7)),
  ...
  divergence: _gsvg2(
    _run([[14,44,34,30,46],[28,34,26,22,36],[42,26,32,24,36],[56,32,22,18,34],[70,22,14,10,24],[84,14,20,12,24],[98,20,16,8,22]])
    + `<line x1="28" y1="20" x2="70" y2="8" stroke="${_GT}" stroke-dasharray="3 2" opacity="0.8"/>`
    + _gdot(28,22,_GT,2.2) + _gdot(70,10,_GT,2.2)
    + `<line x1="8" y1="58" x2="202" y2="58" stroke="#8b85a3" stroke-width="0.8" opacity="0.35"/>`
    + _gp('12,82 28,68 46,88 70,76 92,94 112,86 132,98 152,90 172,102 192,96', _GG, 1.6)
    + `<line x1="28" y1="66" x2="70" y2="74" stroke="${_GG}" stroke-dasharray="3 2" opacity="0.9"/>`
    + _gdot(28,68,_GG,2.2) + _gdot(70,76,_GG,2.2)
    + _glab(198,16,'price ↑',_GT,'end',7) + _glab(198,116,'momentum ↓',_GG,'end',7)),
  'liquidity-sweep': _gsvg2(
    _gl2(34, _GR, true)
    + _run([[14,84,68,64,86],[28,68,52,48,70],[42,52,38,34,54],[56,38,50,36,54],[70,50,40,34,52],[84,40,54,38,58],[98,54,44,40,56],[112,44,50,22,54],[126,50,66,48,68],[140,66,82,64,86],[154,82,92,80,96]])
    + _glab(60,26,'equal highs',_GM,'middle',7) + _glab(112,14,'stops',_GR,'middle',7)),
  fvg: _gsvg2(
    '<rect x="50" y="34" width="152" height="12" rx="1.5" fill="rgba(0,212,212,0.15)"/>'
    + _run([[14,88,76,72,90],[28,76,64,60,78],[56,64,52,46,66],[70,52,26,22,54],[84,26,18,14,34],[98,18,28,16,32],[112,28,36,24,38],[126,36,44,32,46],[140,44,38,34,48],[154,38,26,22,40],[168,26,18,14,28]])
    + _glab(56,58,'1',_GM,'middle',7) + _glab(70,14,'2',_GM,'middle',7) + _glab(88,58,'3',_GM,'middle',7)),
  bos: _gsvg2(
    _gl2(52, _GM, true, 36)
    + _run([[14,96,82,78,98],[28,82,68,64,84],[42,68,54,50,70],[56,54,64,52,68],[70,64,58,54,66],[84,58,40,36,60],[98,40,30,26,42],[112,30,22,18,32]])
    + _gdot(56,66,_GT,2.6) + _garr(120,26,142,16,_GT,1.8)
    + _glab(198,46,'prior high',_GM,'end',7) + _glab(56,82,'higher low',_GT,'middle',7)),
...
// Ichimoku component figures show the WHOLE system with the named line spotlit (_ichiBase).
GL_FIGURES['kijun-sen'] = _gsvg2(_ichiBase('kijun'));
GL_FIGURES['kumo-twist'] = _gsvg2(
  '<polygon points="8,54 56,58 104,58 56,62 8,66" fill="rgba(255,46,136,0.18)"/>'
  + '<polygon points="104,58 152,46 204,38 204,68 152,64" fill="rgba(40,200,120,0.20)"/>'
  + _gp('8,66 56,62 104,58 152,46 204,38','rgba(40,200,120,0.85)',1.6)
  + _gp('8,54 56,58 104,58 152,64 204,68','rgba(255,46,136,0.65)',1.6)
  + _run([[20,88,78,74,90],[36,78,72,68,82],[52,72,64,60,76],[68,64,70,62,74],[84,70,56,52,72],[100,56,44,40,60],[116,44,34,30,46],[132,34,26,22,36]])
  + _gdot(104,58,_GG,3.2)
  + _glab(104,124,'cloud flips',_GG,'middle',7) + _glab(200,30,'projected ahead',_GM,'end',7)
);GL_FIGURES['edge-to-edge'] = _gsvg2(
  '<polygon points="8,36 70,32 140,28 204,26 204,66 140,70 70,76 8,72" fill="rgba(0,212,212,0.13)"/>'
  + _gp('8,36 70,32 140,28 204,26',_GT,1.4) + _gp('8,72 70,76 140,70 204,66',_GR,1.4)
  + _run([[24,92,80,76,94],[40,80,74,70,84],[56,74,62,58,78],[72,62,52,48,66],[88,52,44,40,56],[104,44,36,32,48],[120,36,30,26,40]])
  + _gdot(40,76,_GT,2.8) + _garr(128,30,152,27,_GT,1.6)
  + _glab(40,110,'entry',_GT,'middle',7) + _glab(200,18,'target',_GT,'end',7)
...
// Tenkan / Chikou spotlight the fast line / lagging span within the full system (_ichiBase).
GL_FIGURES['tenkan-sen'] = _gsvg2(_ichiBase('tenkan'));
GL_FIGURES['chikou-span'] = _gsvg2(_ichiBase('chikou'));
// C-Clamp: Tenkan + Kijun compress against the cloud, then price releases in trend.
GL_FIGURES['c-clamp'] = _gsvg2(
  '<polygon points="30,58 112,56 128,50 112,60 30,68" fill="rgba(0,212,212,0.12)"/>'
  + _gp('30,58 70,57 110,56 150,52 180,48',_GG,1.7)
  + _gp('30,68 60,65 90,62 112,60 128,50 148,36 168,26',_GT,1.7)
  + _run([[40,72,66,62,76],[56,68,64,60,72],[72,66,62,58,70],[88,64,60,56,68],[104,62,58,54,66],[124,58,42,38,62],[140,42,30,26,44],[156,30,20,16,32]])
  + _glab(72,92,'compression',_GM,'middle',7) + _glab(152,12,'release',_GT,'middle',7)
...
function _glRefRowHtml(ref, entry) {
  ...
  return `<div class="gl-ref${locked ? ' locked' : ''}" role="button" tabindex="0"
      aria-label="${locked ? 'Locked: ' : 'Open '}${ref.title} — ${GL_COURSE_LABEL[ref.courseNum]}"
      onclick="_glRef(${ref.courseNum},${ref.chapterIdx},${locked},'${entry ? entry.id : ''}')"
...
function renderGlossary(containerId, focusId) {
  ...
  if (_glFocus) {
    ...
    requestAnimationFrame(() => {
      ...
      // gBCR deltas are visual px; scrollTop is layout px — undo the body zoom
      const z = scroller.offsetWidth ? (sr.width / scroller.offsetWidth) : 1;
      scroller.scrollTop += (cr.top - sr.top) / z - 14;
...
/* Global search shortcut — "/" or Cmd/Ctrl+K focuses the header search from
   anywhere in the app (the flagship pattern). Ignored while already typing. */
document.addEventListener('keydown', (e) => {
  ...
  const input = document.getElementById('lt-search-input');
  if (!input || input.offsetParent === null) return;   // hidden (mobile) → leave the key alone
  e.preventDefault();
  input.focus();
  input.select();
});
