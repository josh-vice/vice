...
const TEAL2 = '#00b8b8';
const RED   = '#ff2e88';   // bearish annotation colour — the brand pink, matching down-candles
const GOLD  = '#ffcc00';   // liquidation gold — one gold across charts + the real-data gallery
...
const _ICHI_DEFAULT = { conv: 4, base: 8, span: 16, disp: 8 };
function _computeIchimoku(ohlc, p) {
  p = Object.assign({}, _ICHI_DEFAULT, p || {});
  const n = ohlc.length, d = p.disp;
  const r2 = v => Math.round(v * 100) / 100;
  const tenkan = [], kijun = [], aRaw = [], bRaw = [];
  for (let i = 0; i < n; i++) {
    tenkan.push(r2(_ichimokuMid(ohlc, p.conv, i)));
    kijun.push(r2(_ichimokuMid(ohlc, p.base, i)));
    bRaw.push(r2(_ichimokuMid(ohlc, p.span, i)));
...
function _synthIchiLookback(ohlc, H) {
  ...
  // waypoint offsets from P0 (a couple of ranges + a drift), ending at 0 (= P0)
...
function _computeRSI(ohlc, period) {
  period = period || 6;
...
function _pillText(hex) {
  const p = _hexToRgb(hex).split(',').map(Number);
...
function _computeCumDelta(ohlc) { let a = 0; return ohlc.map(c => (a += (c[1] - c[0]), +a.toFixed(2))); }
...
function _synthVolume(ohlc, opt) {
  opt = opt || {};
...
function mountV2Lesson(hostEl, chapter) {
  ...
  if (lesson && typeof LTRenderer !== 'undefined') {
    ...
    const _colors = {
      bull: localStorage.getItem('lt_bullish_color') || '#00d4d4',
      bear: localStorage.getItem('lt_bearish_color') || '#ff2e88'
    };
    ...
    _v2Player = new LTRenderer(hostEl, lesson, {
      ...
      captionSize: localStorage.getItem('lt_caption_size') || 'md',   // subtitle size (sm/md/lg)
      narration: localStorage.getItem('lt_narration') !== '0',        // narration on/off (default on)
