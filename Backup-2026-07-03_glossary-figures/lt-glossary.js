...
/* Full Ichimoku system for the component glossary figures: candles + Kumo cloud
   (green Senkou A / red Senkou B) + Kijun (gold) + Tenkan (teal) + Chikou (purple).
   Pass a component key ('tenkan'|'kijun'|'chikou'|'cloud') to SPOTLIGHT it — that
   line goes bold / full-opacity and the rest dim — so the figure shows the WHOLE
   system yet points at the one the term names. Labels stay GENERIC (never the term)
   so the figure still works as a Definition→Term flashcard prompt. 'all' = whole system. */
function _ichiBase(hi) {
  const full = !hi || hi === 'all';
  const o = (k) => (full || hi === k) ? 1 : 0.24;
  const w = (k) => hi === k ? 2.8 : 1.6;
  const candles = _gc(44,84,80,76,88)+_gc(60,80,72,68,84)+_gc(76,72,76,70,82)+_gc(92,76,62,58,80)+_gc(108,62,54,50,66)+_gc(124,54,46,42,58);
  const cloud = `<g opacity="${o('cloud')}">`
    + '<polygon points="30,92 64,88 100,82 150,74 150,94 100,96 30,98" fill="rgba(40,200,120,0.20)"/>'
    + _gp('30,92 64,88 100,82 150,74','rgba(40,200,120,0.85)',w('cloud'))
    + _gp('30,98 100,96 150,94','rgba(255,46,136,0.65)',w('cloud')) + '</g>';
  const kijun  = `<g opacity="${o('kijun')}">`  + _gp('36,88 68,80 100,70 130,60',_GG,w('kijun')) + '</g>';
  const tenkan = `<g opacity="${o('tenkan')}">` + _gp('36,84 60,74 76,72 92,64 108,54 130,46',_GT,w('tenkan')) + '</g>';
  const chikou = `<g opacity="${o('chikou')}">` + _gp('8,56 26,48 44,42 58,38',_GC,w('chikou')) + '</g>';
  const cg = `<g opacity="${full ? 1 : 0.5}">${candles}</g>`;
  const lab = hi === 'tenkan' ? _glab(150,44,'fast line',_GT,'end',7)
            : hi === 'kijun'  ? _glab(150,58,'slow line',_GG,'end',7)
            : hi === 'chikou' ? _glab(6,34,'plotted behind',_GC,'start',7)
            : '';
...
const GL_FIGURES = {
  ...
  breakout:      _gsvg(_gl(40,_GR,true)+_gp('10,74 28,46 46,70 64,46 82,66 100,34',_GT,1.8)+_gp('100,34 116,44 132,30 146,14',_GT,1.8)+_gdot(116,44,_GG,2.6)+_garr(132,30,146,15,_GT,1.7)+_glab(8,36,'level',_GR,'start',7)+_glab(150,52,'retest',_GG,'end',7)),
  ...
  ichimoku:      _gsvg(_ichiBase('all')),   // full system: cloud + Tenkan + Kijun + Chikou over price
  ...
  divergence:    _gsvg(_gp('10,46 38,26 64,40 98,20',_GT,1.8)+_gdot(38,26,_GT,2.4)+_gdot(98,20,_GT,2.4)+`<line x1="38" y1="26" x2="98" y2="20" stroke="${_GT}" stroke-dasharray="3 2" opacity="0.7"/>`+`<line x1="6" y1="56" x2="150" y2="56" stroke="#8b85a3" stroke-width="0.8" opacity="0.35"/>`+_gp('10,74 38,66 64,82 98,78',_GG,1.8)+_gdot(38,66,_GG,2.4)+_gdot(98,78,_GG,2.4)+`<line x1="38" y1="66" x2="98" y2="78" stroke="${_GG}" stroke-dasharray="3 2" opacity="0.8"/>`+_glab(150,20,'price ↑',_GT,'end',7)+_glab(150,92,'momentum ↓',_GG,'end',7)),
  ...
  // Break of Structure: an uptrend prints a swing high, pulls back to a higher low,
  // then breaks ABOVE that prior high (dashed level) — continuation confirmed.
  bos:           _gsvg(_gl(40,_GM,true)+_gp('10,88 34,40 58,66 84,50 112,30 140,16',_GT,2)+_gdot(34,40,_GR,2.8)+_gdot(58,66,_GT,2.6)+_garr(100,42,124,26,_GT,1.9)+_glab(150,36,'prior high',_GM,'end',7)+_glab(58,82,'higher low',_GT,'middle',7)),
...
GL_FIGURES['kijun-sen'] = _gsvg(_ichiBase('kijun'));
GL_FIGURES['kumo-twist'] = _gsvg('<polygon points="8,44 58,52 58,60 8,52" fill="rgba(255,46,136,0.16)"/><polygon points="58,60 104,54 150,38 150,62 104,64 58,60" fill="rgba(40,200,120,0.20)"/>'+_gp('8,44 58,52 104,54 150,38','rgba(40,200,120,0.85)',1.6)+_gp('8,52 58,60 104,64 150,62','rgba(255,46,136,0.65)',1.6)+_gdot(58,56,_GG,3.4)+_garr(58,76,58,64,_GG,1.6)+_glab(58,94,'cloud flips',_GG,'middle',7)+_glab(150,32,'forward',_GM,'end',7));
GL_FIGURES['edge-to-edge'] = _gsvg('<polygon points="8,44 60,44 150,38 150,66 60,72 8,72" fill="rgba(0,212,212,0.14)"/>'+_gp('8,44 60,44 150,38',_GT,1.4)+_gp('8,72 60,72 150,66',_GR,1.4)+_gc(40,80,72,68,84)+_garr(52,72,120,44,_GT,2)+_gdot(44,72,_GT,2.8)+_glab(28,92,'enter edge',_GT,'start',7)+_glab(150,34,'target edge',_GT,'end',7));
...
GL_FIGURES['tenkan-sen'] = _gsvg(_ichiBase('tenkan'));
GL_FIGURES['chikou-span'] = _gsvg(_ichiBase('chikou'));
// C-Clamp: Tenkan + Kijun compress against the cloud, then price releases in trend.
GL_FIGURES['c-clamp'] = _gsvg('<polygon points="8,52 96,50 96,62 8,60" fill="rgba(0,212,212,0.14)"/>'+_gp('8,44 96,52',_GG,1.7)+_gp('8,62 96,56',_GT,1.7)+_gc(40,58,54,50,62)+_gc(58,57,53,49,61)+_gc(76,56,52,48,60)+_gp('96,54 140,16',_GT,2.4)+_garr(122,32,140,16,_GT,1.9)+_glab(50,78,'compress',_GT,'middle',7)+_glab(116,44,'release',_GT,'start',7));
