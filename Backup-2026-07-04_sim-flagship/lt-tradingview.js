  ...
  function load() {
    ...
    _loading = new Promise(function (resolve, reject) {
      ...
      const done = () => {
        ...
        else reject(new Error('TradingView loaded but widget API missing'));
      };
      if (!s) {
        ...
        s.onload = done;
        s.onerror = () => { _loading = null; reject(new Error('TradingView script failed to load')); };
        document.head.appendChild(s);
      } else if (g.TradingView && g.TradingView.widget) {
        done();
      } else {
        s.addEventListener('load', done, { once: true });
        s.addEventListener('error', () => { _loading = null; reject(new Error('TradingView script failed to load')); }, { once: true });
      }
      setTimeout(() => { if (!(g.TradingView && g.TradingView.widget)) { _loading = null; reject(new Error('TradingView load timed out')); } }, TIMEOUT);
    });
    return _loading;
  }
