...
/* ── Per-chapter STEP MODEL ──────────────────────────────────────────────────
   Every chapter runs Introduction · Lesson · Quiz. The Introduction page carries the
   overview text alongside the chapter's chart (or concept diagram / course roadmap);
   the Lesson is the animated v2 player; the Quiz follows. All step math (dispatch, nav
   bounds, quiz detection, progress, pills, routing) flows through these helpers. */
function chapterStepKeys(chapter)   { return ['intro', 'lesson', 'quiz']; }
function chapterStepLabels(chapter) { return STEP_LABELS; }
...
function mountV2Lesson(hostEl, chapter) {
  ...
  if (lesson && typeof LTRenderer !== 'undefined') {
    ...
    const _colors = {
      ...
      bear: localStorage.getItem('lt_bearish_color') || '#f2f2f2'
    };
    // onComplete: the player's end-of-lesson "continue" cue advances to the Quiz step.
    _v2Player = new LTRenderer(hostEl, lesson, { onComplete: function () { navigate('next'); }, colors: _colors });
    return true;
  }
  return false;
}
...
function _ltParseRoute() {
  try {
    ...
    if (p.has('c') && p.has('ch')) {
      ...
      if (c >= 1 && c <= 4 && ch >= 0) {
        return { kind: 'lesson', course: c, chapter: ch, step: (s >= 0 && s <= 2) ? s : 0 };
      }
    }
