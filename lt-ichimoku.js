...
(function () {
  ...
  function _chartSvg() {
    ...
    var ohlc = _series();
    var ich = TA ? TA.computeIchimoku(ohlc, TA.ICHI_DEFAULT) : null;   // shares engine periods (widened span = thicker cloud)
    ...
    var divider = '<line x1="' + xNow.toFixed(1) + '" y1="' + (PT - 4) + '" x2="' + xNow.toFixed(1) + '" y2="' + (H - PB + 2) + '" stroke="#8b85a3" stroke-width="1" stroke-dasharray="2 3" opacity=".5"/>'
      + '<text x="' + (xNow + 4).toFixed(1) + '" y="' + (PT + 2) + '" fill="#8b85a3" font-size="8.5" font-family="Geist Mono,monospace">cloud →</text>';
  ...
  function _ichStyles() {
      ...
      ".ich-wrap{width:100%;max-width:600px;margin:0 auto;font-family:'Geist Mono',ui-monospace,monospace;}",
