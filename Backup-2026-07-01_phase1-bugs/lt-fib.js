...
(function () {
  ...
  function _fibStyles() {
    if (document.getElementById('lt-fib-styles')) return;
    var s = document.createElement('style'); s.id = 'lt-fib-styles';
    s.textContent = [
  ...
  function renderFibDemo(containerId) {
    ...
    el._fibTimer = setInterval(function () {
      if (!document.body.contains(el)) { stopFibDemo(containerId); return; }
      i = (i + 1) % PHASES.length;
      paint(i, true);
    }, STEP_MS);
  }
