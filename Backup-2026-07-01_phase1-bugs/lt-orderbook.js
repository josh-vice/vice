...
(function () {
  ...
  var STEP_MS = 1300;                 // dwell per phase
  var IDS = ['a4', 'a3', 'a2', 'a1', 'lim', 'b1', 'b2', 'b3', 'b4'];

  function _obStyles() {
    if (document.getElementById('lt-orderbook-styles')) return;
    var s = document.createElement('style');
    s.id = 'lt-orderbook-styles';
    s.textContent = [
      ...
      ".ob-depth{position:absolute;top:1px;bottom:1px;right:0;width:0;z-index:0;border-left:1px solid transparent;",
      "  transition:width .6s cubic-bezier(.4,0,.2,1),opacity .4s ease;}",
      ".ob-ask .ob-depth{background:linear-gradient(90deg,transparent,color-mix(in srgb,var(--ob-ask) 26%,transparent));border-left-color:color-mix(in srgb,var(--ob-ask) 40%,transparent);}",
      ".ob-bid .ob-depth{background:linear-gradient(90deg,transparent,color-mix(in srgb,var(--ob-bid) 24%,transparent));border-left-color:color-mix(in srgb,var(--ob-bid) 38%,transparent);}",
  ...
  function renderOrderBookDemo(containerId) {
    ...
    el._obTimer = setInterval(function () {
      if (!document.body.contains(el)) { stopOrderBookDemo(containerId); return; }
      i = (i + 1) % phases.length;
      phases[i](st);
      apply(st, i, true);
    }, STEP_MS);
