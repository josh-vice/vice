/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ltCandles(start, legs, opts)
     ...
     legs  : [{ to, bars, reject? }, ...]
             ...
             ⚠ reject ALWAYS follows the LEG direction: an UP-leg reject grows the
             UPPER wick, a DOWN-leg reject the LOWER wick. To draw e.g. a hanging
             man (lower wick at a top), put the reject on a small DOWN-leg candle —
             a reject on the up-leg gives a shooting-star shape instead.
     opts  : { seed=7, wick=0.5, noise=0.4 }
             ...
             ⚠ ~12% of candles get an occasional LONG wick (freak-wick branch below),
             sized off the bar's typical move — on steep 1-bar legs this can punch
             many points through an annotated level. Whenever a chart's seed/legs/
             wick change, RE-VERIFY the generated candles against every markLine /
             markArea / pinned label (node harness or live), never eyeball-only.
