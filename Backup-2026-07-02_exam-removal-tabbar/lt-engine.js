...
// longer exist; counting those unbounded pushes completion past 100% on the home
// screen and can falsely satisfy the all-courses-complete final-exam gate.
  ...
  if (direction === 'next') {
    ...
    if (atLastStep) {
      ...
      if (allChaptersComplete()) {
        if (examUnlockedNow()) {   // all four courses done → the comprehensive final unlocks
          ...
          setTimeout(() => showToast('<i data-lucide="award" style="width:14px;height:14px;"></i> All four courses complete! The Final Exam is now unlocked.'), 800);
        } else {
          setTimeout(() => showToast('<i data-lucide="check-circle" style="width:14px;height:14px;"></i> Course complete! Finish all four courses to unlock the Final Exam.'), 800);
        }
    ...
    if (step > lastStepIdx(CHAPTERS[ch])) {
      ...
      if (ch >= CHAPTERS.length) {
        ...
        updateProgressUI();
        updateSidebar();
        showToast(examUnlockedNow() ? 'All four courses complete — take the Final Exam!' : 'Course complete!', 3000, 'award');
        return;
      }
    }
  } else {
...
/* Settings' "Mark Complete" / "Reset" writes a course's progress straight to
   ...
   and the sidebar/exam-unlock reflect it immediately. */
  ...
  nav.innerHTML += `
    <div class="sidebar-exam-slot">
      <button class="btn-exam" id="start-exam-btn" onclick="startExam()" ${examUnlocked ? '' : 'disabled'}
              title="${examUnlocked ? 'Take the comprehensive final exam (60 questions)' : 'Complete all four courses to unlock the final exam'}">
...
function _examPool() {
  // The comprehensive final exam: 60 curated quiz questions (15/course, hardest).
  // Falls back to a per-course pool only if the final-exam module failed to load.
  return (typeof LT_FINAL_EXAM !== 'undefined' && LT_FINAL_EXAM.length)
    ? LT_FINAL_EXAM
    : courseExamPool(getActiveCourseNum());
}

const EXAM_LENGTH      = 10;   // (legacy — retained for compatibility; the final exam shows the full set)
const EXAM_CHART_QUOTA = 4;    // (legacy)
...
// The final exam is the FULL curated set (all 60), in course order (a coherent
// Course 1→4 progression), every attempt. Answer order still re-shuffles per
// attempt (seeded in renderExamQuestion), so the choices can't be memorised.
function _buildExamSet() {
  return _examPool().slice();
}
...
/* The comprehensive final exam unlocks once EVERY course is fully complete, and
   stays unlocked after the first attempt (a single key, not per-course). */
const FINAL_EXAM_DONE_KEY = 'lt_examDone_final';
function finalExamDoneBefore() { try { return localStorage.getItem(FINAL_EXAM_DONE_KEY) === '1'; } catch (_) { return false; } }
function _courseCompletedCount(n) {
  if (n === getActiveCourseNum()) return completedCount();                 // active course: live in-memory count (already index-bounded)
  const prog = (typeof window.ltGetCourseProgress === 'function') ? (window.ltGetCourseProgress(n) || {}) : {};
  const def = _courseDef(n);
  return _ltCountCompleted(prog, def && def.chapters ? def.chapters.length : Infinity);
}
function allCoursesComplete() {
  return LT_COURSES.every(c => c.chapters && c.chapters.length && _courseCompletedCount(c.num) >= c.chapters.length);
}
function examUnlockedNow() { return allCoursesComplete() || finalExamDoneBefore(); }

window.startExam = startExam;
function startExam() {
  if (!examUnlockedNow()) {
    showToast('Complete all four courses to unlock the Final Exam.', 3000, 'lock');
    return;
  }
...
function finishExam() {
  ...
  try { localStorage.setItem(FINAL_EXAM_DONE_KEY, '1'); } catch (_) {}   // keep the final exam unlocked for revisits
...
window.showExamAnswerKey = function() {
  ...
  const _finalPool = (typeof LT_FINAL_EXAM !== 'undefined' && LT_FINAL_EXAM.length) ? LT_FINAL_EXAM : null;
  const pools = LT_COURSES.map(c => ({ n: c.num, name: _examNames[c.num],
    pool: _finalPool ? _finalPool.filter(q => q.course === c.num) : c.exam }));
    ...
    return `
      <div class="ak-course" style="--ak-accent:${accent}">
        <div class="ak-course-head">
          ...
          <span class="ak-course-name">${esc(String(p.name).replace(/^Course\s*\d+\s*[:—–-]\s*/i, ''))}</span>
  ...
  area.innerHTML = `
    <div class="ak-wrap">
      <div class="ak-head">
        ...
        <div class="ak-title">Final Exam Answer Key</div>
        <div class="ak-sub">${total} questions — the comprehensive Final Exam (${_finalPool ? 'the 15 hardest quiz questions per course' : 'per-course pools'}), correct answers marked. Review only.</div>
      </div>
      ${sections}
    </div>`;
...
window.showCertificate = function(correct, total) {
  ...
  if (courseEl) courseEl.textContent = (typeof LT_FINAL_EXAM !== 'undefined' && LT_FINAL_EXAM.length) ? 'Liquidity Theory · All Four Courses' : getCourseName();
...
function wireEvents() {
  ...
  // Escape closes the full-screen dialogs (exam via its own Exit path, cert directly) —
  // matches the on-screen buttons; the glossary lightbox + settings modal handle their own.
  document.addEventListener('keydown', e => {
    if (e.key !== 'Escape') return;
    const cert = document.getElementById('cert-screen');
    if (cert && !cert.classList.contains('hidden')) { cert.classList.add('hidden'); return; }
    const exam = document.getElementById('exam-screen');
    if (exam && !exam.classList.contains('hidden') && typeof window.exitExam === 'function') window.exitExam();
  });
...
/* ══════════════════════════════════════════════════════════════════════════
   MOBILE TAB BAR — iOS-style bottom navigation, phones only (CSS gates it to
   ≤768px; desktop never sees it). Injected once at boot; setHomeNavActive()
   keeps the active tab in sync and hides the bar inside course/exam views
   (iOS "hidesBottomBarWhenPushed" — the Back/Next footer owns the bottom there).
   ══════════════════════════════════════════════════════════════════════════ */
window.ltTabCourse = function() { ltOpenCourse(getActiveCourseNum()); };

function _ltInjectTabbar() {
  if (document.getElementById('lt-tabbar')) return;
  const nav = document.createElement('nav');
  nav.id = 'lt-tabbar';
  nav.className = 'lt-tabbar';
  nav.setAttribute('aria-label', 'Primary');
  nav.innerHTML = `
    <button class="lt-tab" data-tab="home"       onclick="showHome()"       aria-label="Home"><i data-lucide="home"></i><span>Home</span></button>
    <button class="lt-tab" data-tab="course"     onclick="ltTabCourse()"    aria-label="Course"><i data-lucide="book-open"></i><span>Course</span></button>
    <button class="lt-tab" data-tab="glossary"   onclick="showGlossary()"   aria-label="Glossary"><i data-lucide="book-marked"></i><span>Glossary</span></button>
    <button class="lt-tab" data-tab="flashcards" onclick="showFlashcards()" aria-label="Flashcards"><i data-lucide="layers"></i><span>Cards</span></button>
    <button class="lt-tab" data-tab="settings"   onclick="showSettings()"   aria-label="Settings"><i data-lucide="settings"></i><span>Settings</span></button>`;
  document.body.appendChild(nav);
  if (typeof lucide !== 'undefined') lucide.createIcons();
  _ltSyncTabbar();
}

// Map every state.view to a tab (views without their own tab highlight the hub
// they're reached from) — course/exam hide the bar entirely.
function _ltSyncTabbar() {
  const tb = document.getElementById('lt-tabbar');
  if (!tb) return;
  const hidden = state.view === 'course' || state.view === 'exam';
  const map = { home: 'home', glossary: 'glossary', flashcards: 'flashcards',
                settings: 'settings', answerkey: 'settings',
                simulator: 'home', community: 'home' };
  const cur = map[state.view] || null;
  tb.classList.toggle('lt-tabbar--hidden', hidden);
  document.body.classList.toggle('has-tabbar', !hidden);
  tb.querySelectorAll('.lt-tab').forEach(b =>
    b.classList.toggle('lt-tab--active', b.dataset.tab === cur));
}
...
function setHomeNavActive() {
  ...
  _ltSyncTabbar();                                            // mobile bottom tab bar follows every view change
}
