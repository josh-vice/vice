...
(function (g) {
  ...
  var CHARTS = {
    ...
    range_bound: function () {
      ...
      // back INSIDE — a defended edge, not an overshoot/sweep.
      var cs = grow(60, [
    ...
    uptrend: function (params) {
      ...
      var cs = grow(40, [
        ...
        { to: 58, bars: 2, reject: 1.5 }, // hl2     (idx 9)  retests 58 as support
    ...
    horizontal_sr: function (params) {
      ...
      if (as === 'resistance') {
        ...
        ]); // touch ends: 1, 5, 9, 13, 15 ; break ends idx 18
        return {
          candles: csR,
          anchors: {
            ...
            touch5: { type: 'point', i: 15, price: R },   // the touch that breaks
            breakout: { type: 'point', i: 18, price: 74 },
          ...
          stages: { hold: 16, break: csR.length, all: csR.length },
      ...
      return {
        ...
        anchors: {
          ...
          touch5: { type: 'point', i: 15, price: S },   // the touch that breaks
          breakdown: { type: 'point', i: 18, price: 36 },
          stop: { type: 'level', price: 54 },           // for a buy-the-support long, just under S
        ...
        stages: { hold: 16, break: cs.length, all: cs.length },
    ...
    sr_flip: function () {
      var L = 55;
      var cs = grow(40, [
        { to: 54, bars: 3, reject: 2 },   // test resistance #1   (idx 2)
        { to: 44, bars: 2 },              // pull back (swing low) (idx 4)
        { to: 54, bars: 2, reject: 1.5 }, // test resistance #2    (idx 6)  rejected again
        { to: 66, bars: 3 },              // BREAK above           (idx 9)
        { to: 56, bars: 2, reject: 1.5 }, // retest, holds support (idx 11) higher low
    ...
    consolidation_breakout: function (params) {
      ...
      if (dir === 'down') {
        // tag high(58)@1,9 / low(48)@3,7,11 → 5th test (low) breaks down
        var csD = grow(52, [
          { to: HI, bars: 2, reject: 1 },   // touchHi1 (idx1)
          { to: LO, bars: 2, reject: 1 },   // touchLo1 (idx3)
          { to: HI, bars: 2, reject: 1 },   // touchHi2 (idx5)
          { to: LO, bars: 2, reject: 1 },   // touchLo2 (idx7)
          { to: HI, bars: 2, reject: 1 },   // touchHi3 (idx9)
          { to: LO, bars: 2 },              // touchLo3 (idx11) 5th test at the edge
          { to: LO - HGT, bars: 4 }         // BREAK down (idx15) target = LO - height
        ]);
        return {
          candles: csD,
          anchors: {
            ...
            entry: { type: 'point', i: 13, price: LO },        // first bar below the range low
            breakout: { type: 'point', i: 15, price: LO - HGT },
            stop: { type: 'level', price: HI },                 // opposite edge = invalidation
            target: { type: 'level', price: LO - HGT }          // measured move (height projected)
          },
          stages: { consolidate: 12, breakout: csD.length },
          order: ['consolidate', 'breakout'], yhint: yspan(csD, 0.1),
          volume: (params && params.volume) ? breakoutVol(csD, 12) : undefined
        };
      }
      // tag low(48)@3,7 / high(58)@1,5,9 → 5th test (high) breaks up
      var cs = grow(52, [
        { to: HI, bars: 2, reject: 1 },   // touchHi1 (idx1)
        { to: LO, bars: 2, reject: 1 },   // touchLo1 (idx3)
        { to: HI, bars: 2, reject: 1 },   // touchHi2 (idx5)
    ...
    engulfing_bullish: function (params) {
      ...
      // spike on the engulfing candle ≈ 2× the prior down day (no prior-bar suppression,
      // so the label "2× volume" matches the drawn bar height).
      if (params && params.volume) out.volume = vol(cs, { 3: 2.0 });
      return out;
    },
    ...
    engulfing_bearish: function (params) {
      ...
      if (params && params.volume) out.volume = vol(cs, { 3: 2.0 });   // ≈2× prior, matches the label
      return out;
    },
