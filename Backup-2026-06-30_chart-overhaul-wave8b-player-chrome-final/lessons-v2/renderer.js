...
<div class="ltp2-complete-title"></div>\
  ...
  LTRenderer.prototype._resolveChartAnno = function (geom, item, revealCount) {
    ...
    if (item.kind === 'heatmap') {
      ...
      return { id: 'heatmap:' + item.of + '-' + item.to, kind: 'heatmap', p1: H1.price, p2: H2.price, tone: item.tone || 'long', peak: (item.peak != null ? item.peak : 0.5), label: item.label };
    }
  ...
  LTRenderer.prototype._showPanel = function (p, animate) {
    ...
    (p.lines || []).forEach(function (ln, idx) {
      ...
      self._lineTimers.push(setTimeout(function () { li.classList.add('on'); }, (110 + idx * 165) / sp));
    });
  ...
  LTRenderer.prototype._drawCloud = function (P, geom, reveal, revealA) {
    ...
    // faint Senkou A (green) / Senkou B (orange) edge lines, like a real platform
    [{ s: B, c: 'rgba(242,61,92,.55)' }, { s: A, c: 'rgba(40,200,120,.6)' }].forEach(function (edge) {
      ctx.strokeStyle = edge.c; ctx.lineWidth = 1; ctx.beginPath(); var started = false;
      for (var j = 0; j <= end; j++) {
  ...
  LTRenderer.prototype._drawOverlays = function (P, geom, reveal, revealA) {
    ...
    geom.overlays.forEach(function (ov) {
      ...
      if (ov.label && started) {
        ctx.fillStyle = ov.color; ctx.font = '10px Cascadia Code, JetBrains Mono, monospace';
  ...
  LTRenderer.prototype._drawSub = function (d, P, revealA) {
    ...
    ctx.fillStyle = C.muted; ctx.font = '10px Cascadia Code, JetBrains Mono, monospace';
    ...
    if (S.type === 'volume') {
      ...
      if (S.bands) S.bands.forEach(function (b) {
        ...
        ctx.fillStyle = C.muted; ctx.font = '9px Cascadia Code, JetBrains Mono, monospace';
  ...
  LTRenderer.prototype._drawMarkerLabels = function (P) {
    ...
    ctx.font = FS + 'px Cascadia Code, JetBrains Mono, monospace';
    ...
    ctx.font = '11px Cascadia Code, JetBrains Mono, monospace'; ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic';
    return anim;
  };
  ...
  LTRenderer.prototype._paintLabel = function (L, FS) {
    ...
    ctx.fillStyle = L.color; ctx.font = FS + 'px Cascadia Code, JetBrains Mono, monospace';
  ...
  LTRenderer.prototype._anno = function (P, an, av) {
    ...
    if (an.kind === 'level') {
      ...
      if (an.label) {
        ctx.font = '700 11px Cascadia Code, JetBrains Mono, monospace';
        var padX = 6, padY = 3, lh = 12, tw = ctx.measureText(an.label).width;
        ...
        ctx.fillText(an.label, pxL + padX, pyT + ph / 2 + 0.5);
        ctx.globalAlpha = 1;
      }
      ...
      if (an.label) {
        ...
        ctx.font = '700 10px Cascadia Code, JetBrains Mono, monospace';
        var zpadX = 6, ztw = ctx.measureText(an.label).width, zpw = ztw + zpadX * 2, zph = 17;
        ...
        ctx.fillText(an.label, zx + zpadX, zy + zph / 2 + 0.5);
        ctx.globalAlpha = 1;
      }
      ...
      for (var hr = 0; hr < rows; hr++) {
        ...
        ctx.globalAlpha = av * (0.05 + 0.55 * dens);
        ctx.fillStyle = 'rgb(' + rgb + ')';
        ctx.fillRect(P.x0, yTop + hf * band, P.x1 - P.x0, band / rows + 1);
      }
      ctx.globalAlpha = av;
      if (an.label) {
        ctx.fillStyle = C.ink; ctx.font = '10px Cascadia Code, JetBrains Mono, monospace';
        ctx.textAlign = 'left'; ctx.textBaseline = 'middle';
        this._haloText(an.label, P.x0 + 6, yTop + an.peak * band);
      }
      ...
      ctx.strokeStyle = an.style === 'resistance' ? C.bear : C.teal; ctx.lineWidth = 1.7;
      ctx.beginPath(); ctx.moveTo(ax, ay); ctx.lineTo(lx, ly); ctx.stroke();
      if (an.label && av > 0.55) {
        ctx.fillStyle = an.style === 'resistance' ? C.bear : C.teal; ctx.font = '11px Cascadia Code, JetBrains Mono, monospace';
  ...
  LTRenderer.prototype._drawCandle = function (now, d) {
    ...
    for (var j = 0; j < d.annos.length; j++) {
      ...
      if (an.kind === 'region') {
        ...
        ctx.fillStyle = C.ink; ctx.font = '12px Cascadia Code, JetBrains Mono, monospace';
        ctx.textAlign = 'left'; ctx.textBaseline = 'middle';
