...
const LT_CHAPTERS_3 = [
     ...
     ═══════════════════════════════════════════════════════════════════════ */
  {
    ...
    quiz: {
      ...
      chart: {
        ...
        labels: ltLabels(14),
        ohlc: ltCandles(98, [
          { to: 86, bars: 9, reject: 2 },
          { to: 107, bars: 5 }
        ], { seed: 139, wick: 0.4 }),
        markLines: [
          { yAxis: 86, label: "Limit Buy @ 86", color: "#00d4d4" }
        ],
        markAreas: [
          { y0: 84, y1: 89, label: "", color: "rgba(0,212,212,0.07)" }
        ]
      },
      revealMarkPoints: [
     ...
     ═══════════════════════════════════════════════════════════════════════ */
  {
    ...
    quiz: {
      ...
      revealMarkPoints: [
        ...
        { dataIndex: 10, label: "New Short Opened — Danger", position: "bottom", color: "#ff2e88" },
        { dataIndex: 12, label: "Doubled Loss!",             position: "bottom", color: "#ff2e88" }
      ],
     ...
     ═══════════════════════════════════════════════════════════════════════ */
  {
    ...
    lessonChart: {
      ...
      subPanel: {
        ...
        points: [[0, 100], [7, 150], [10, 158], [15, 215]]
      },
      markPoints: [
     ...
     ═══════════════════════════════════════════════════════════════════════ */
  {
    ...
    // Bespoke interactive section (its own step after the Lesson) — a leverage slider
    // that moves the liquidation line (lt-liqlab.js). Charts untouched.
    demo: {
      kind: "liqlab",
      label: "Liquidation Lab",
      after: "lesson",
      heading: "Feel How Leverage Moves Your Liquidation",
      body: "Leverage doesn't change your entry — it changes how far price has to move against you before the exchange force-closes the position. Drag the slider and watch the liquidation line creep toward your entry as leverage climbs. The higher you go, the smaller the move that wipes you out.",
      bullets: [
        "Liquidation distance ≈ 100% ÷ leverage (isolated) — 10× liquidates near −10%, 50× near −2%",
        "Past ~12×, liquidation sits <strong>inside</strong> a sensible stop — you're force-closed before your stop can save you",
        "Size the position first, THEN pick leverage so liquidation sits well beyond your stop — never the reverse",
        "Illustrative math (no fees/funding) — real liquidation is a touch closer"
      ]
    },
     ...
     ═══════════════════════════════════════════════════════════════════════ */
  {
    ...
    introChart: {
      ...
      type: "candlestick",
      labels: ltLabels(16),
      // Entry (84) is a clean tag of the consolidation floor that then rejects, and the
      // rip lands EXACTLY on the target (114) instead of overshooting. Stop/Liq are
      // tightened just under the consolidation low to kill the dead whitespace band.
      ohlc: ltCandles(86, [
        { to: 84, bars: 4 },
        { to: 86, bars: 3 },
      ...
      markLines: [
        { yAxis: 83, label: "Entry ($134 scaled)", color: "#00d4d4" },
        { yAxis: 80, label: "Stop Loss ($125)",    color: "#ff2e88" },
        { yAxis: 78, label: "Liquidation ($115)",  color: "#ffcc00" },
     ...
     ═══════════════════════════════════════════════════════════════════════ */
  {
    ...
    introChart: {
      // Zone re-anchored to the actual DBS candle (idx9: open 85.28, low 79.36): per
      // the definition, bottom = lowest wick (79.4), top = highest open of the down
      // candle (85.3). The old 77–82 box floated below every wick and sliced the body.
      title: "DBS Zone — Explosive Breakout Defines Maximum Strength",
      markLines: [ { yAxis: 85.3, label: "DBS Top — Highest Open of Down Candle", color: "#00d4d4" }, { yAxis: 79.4, label: "DBS Bottom — Lowest Wick", color: "#00d4d4" } ],
      type: "candlestick",
      labels: ltLabels(18),
      ohlc: ltCandles(108, [
        { to: 80,  bars: 10 },
      ...
      markAreas: [
        { y0: 79.4, y1: 85.3, label: "DBS Zone", color: "rgba(0,212,212,0.08)" }
      ],
      markPoints: [
        { dataIndex:  9, label: "Single Down Candle = DBS",  position: "bottom" },
        { dataIndex: 10, label: "Explosive Breakout Up!",    position: "top"    },
     ...
     ═══════════════════════════════════════════════════════════════════════ */
  {
    ...
    introChart: {
      ...
      markLines: [ { yAxis: 106, label: "Max Acceptable Drawdown — below = revamp", color: "#ff2e88" } ],
      type: "candlestick",
      labels: ltLabels(18),
      ohlc: ltCandles(100, [
        { to: 113, bars: 7 },
    ...
    quiz: {
      ...
      markLines: [ { yAxis: 80, label: "Max Acceptable Drawdown (predefined)", color: "#ff2e88" } ],
        type: "candlestick",
        cutIndex: 8,
        labels: ltLabels(16),
        ohlc: ltCandles(100, [
     ...
     ═══════════════════════════════════════════════════════════════════════ */
  {
    ...
    lessonChart: {
      ...
      markLines: [ { yAxis: 92, label: "Actual Exit — Slippage Below Stop", color: "#ff2e88" },
        ...
        { yAxis: 116, label: "Target $15,375 (never reached)",             color: "#00d4d4" }
      ],
      markPoints: [
     ...
     ═══════════════════════════════════════════════════════════════════════ */
  {
    ...
    quiz: {
      ...
      revealMarkPoints: [
        ...
        { dataIndex:  8, label: "Entry Missed — Zone Left", position: "bottom", color: "#ff2e88" },
...
const LT_EXAM_QUESTIONS_3 = [
  ...
  { chapterTitle: 'Trading Ranges',
    ...
    // Final 6-bar leg bounces UP off the tagged Range Low so the engine's appended
    // trail (a 5-bar-slope continuation) resolves UPWARD — agreeing with the "buy
    // support" answer — instead of breaking down through 84 as the old last leg did.
    chart: { type:'candlestick', labels: ltLabels(18),
      ohlc: ltCandles(95, [{ to:84, bars:3, reject:0.6 }, { to:96, bars:3, reject:0.6 }, { to:84, bars:3, reject:0.6 }, { to:96, bars:2, reject:0.6 }, { to:84, bars:1 }, { to:94, bars:6 }], { seed: 341, wick: 0.4 }),
      markAreas: [
        { y0: 94, y1: 98, label: 'Range High', color: 'rgba(255,46,136,0.06)' },
  ...
  { chapterTitle: 'Identifying Access Points',
    ...
    chart: { type:'candlestick', labels: ltLabels(14),
      ...
      markLines: [{ yAxis: 101, label: 'Resistance', color: '#ff2e88' }] },
    answers: [
  ...
  { chapterTitle: 'Trading S/R — DBS/SSR',
    ...
    // Pullback now GENTLY steps back into the zone (idx8 tags ~95/into the band) and a
    // 6-bar leg rises to 104, so the engine's 5-bar-slope trail continues UP and the
    // buyers-defend-the-zone answer is shown. The old {to:92} last leg let the trail
    // collapse to ~80, failing the very zone the answer says to buy.
    chart: { type:'candlestick', labels: ltLabels(15),
      ohlc: ltCandles(95, [{ to:90, bars:3 }, { to:112, bars:3 }, { to:95, bars:3 }, { to:104, bars:6 }], { seed: 343, wick: 0.4 }),
  ...
  { chapterTitle: 'Leverage Done Right',
    ...
    // Palette fixed: Liquidation is now GOLD (was pink, indistinguishable from the
    // pink Stop). Price wobbles down toward the stop (dips to ~96.5) but never through
    // it, so the buffer between stop and liq is demonstrated; 96–92 band shaded.
    chart: { type:'candlestick', labels: ltLabels(11),
      ohlc: ltCandles(100, [{ to:103, bars:3 }, { to:97, bars:3 }, { to:104, bars:5 }], { seed: 344, wick: 0.35 }),
      markAreas: [{ y0: 92, y1: 96, label: 'Stop→Liq Buffer', color: 'rgba(255,204,0,0.06)' }],
      markLines: [
        { yAxis: 100, label: 'Entry 100',       color: '#00d4d4' },
        { yAxis: 96,  label: 'Stop 96',         color: '#ff2e88' },
        { yAxis: 92,  label: 'Liquidation 92',  color: '#ffcc00' } ] },
    answers: [
  ...
  { chapterTitle: 'Trading S/R — DBS/SSR',
    ...
    // Retest now tags the zone BODY (idx8 = 100) and a 6-bar leg rolls DOWN to 90, so
    // the engine's 5-bar-slope trail continues DOWN and the "sellers defend, short"
    // answer resolves on-screen — the old {to:98} last leg stalled inside the zone and
    // let the trail drift further in.
    chart: { type:'candlestick', labels: ltLabels(15),
      ohlc: ltCandles(95, [{ to:100, bars:2 }, { to:100, bars:2 }, { to:82, bars:3 }, { to:100, bars:2 }, { to:90, bars:6 }], { seed: 345, wick: 0.4 }),
      markAreas: [{ y0: 97, y1: 102, label: 'Supply Zone', color: 'rgba(255,46,136,0.07)' }] },
    answers: [
