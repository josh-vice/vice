...
- `Backup-2026-06-17_0004/` — **latest**, captures this session's work: Scenario+module-Quiz step,
  adaptive results panel, lesson chart label pills/collision/hover, lesson-complete button removal
  (`lt-engine.js 3.100.1`, `lt-styles.css 1.98.1`, `lessons-v2/renderer.js 1.5.1`, `player.css 1.4.1`,
  `lt-module-quizzes.js 1.0.0`).
...
## ▶ NEXT SESSION — pending work (priority order)

### 0. ✅ DONE — v2 IS THE ONLY LESSON SYSTEM; v1 RECON + rebuild/ TREE DELETED (2026-06-16)
The from-scratch rebuild of **all 83+ lesson modules** is now the **ONLY lesson renderer on the
live app** (full swap, shipped silent). v2 (editable content files / named chart vocabulary / a
dependency-free Canvas renderer) renders the lesson step — real Ichimoku clouds, proprietary tool
palettes (Crayons/Genie/etc.), sentiment sub-panels, RSI/volume panels, and more.
The **v1 recon player AND the written-lesson path were removed from `lt-engine.js`**, and the
entire **`rebuild/` tree + `lt-recon-player.js` + `player/` (~335 MB) were deleted** (API key
preserved at workspace-root `.anthropic_key`, git-/vercel-ignored). The last orphan, C2
"Divergences", was authored as a v2 lesson (`lessons-v2/lessons/Course2/18_Divergences.js`).
Verified in the Claude Preview, **0 console errors across all four courses**; OpenClaw auto-deploys.
👉 Full write-up: **`lessons-v2/MIGRATION_HANDOFF.md`** ("Status: MIGRATED LIVE + V1 FULLY REMOVED").
Backups: `Backup-2026-06-16_1716/`; deleted tracked files recoverable from git HEAD (NOT committed).

### Other pending work
1. **DEPLOY is handled automatically by OpenClaw's Vercel integration — do NOT run a manual
   `vercel --prod`.** (Confirmed by the user 2026-06-15.) `.vercelignore` ships the `lessons-v2/`
   runtime (JS + `player.css`) and excludes the `*.md` docs + `course*.html`/`preview.html` hubs +
   `rebuild`/`player`/`.agents`/`.impeccable`/`PRODUCT.md`/`.anthropic_key`. **None of this session's
   work is git-committed** (OpenClaw deploys from the working tree); commit if you want git as the net.
2. **`impeccable` skill is installed** (`.claude`/`.agents`, with a PostToolUse design-detector hook;
   `/impeccable` is NOT a registered slash command this session — run its flows via the reference docs,
   like init/critique/audit were). `PRODUCT.md` exists. Open impeccable follow-ups from the reviews:
   - `/impeccable document` → generate **DESIGN.md** from the token system (recommended; nothing captures it yet).
   - **Lesson-player affordance hint** (`/impeccable onboard`) — the critique's top P2: nothing tells
     first-timers about ←/→, click-to-advance, or Play.
   - Tokenize a couple stray `player.css` hexes (`#6f6cff`/`#ffb454`) (`/impeccable extract`).
   - **Simulator candles** are still inconsistent (`lt-sim-ui.js:757` solid-up vs `:933` hollow) — align
     to the site's hollow-up/filled-down + cyan/pink if you want full-app uniformity.
3. **(Optional) Per-beat TTS for v2** — lessons are silent; each beat has a spoken-ready `say` and the
   renderer has a `_cueAudio` hook, so audio can be generated without re-authoring.
4. **(Optional) Glossary term-flash** only scans the **intro** HTML (the lesson is canvas). Could extend
   to re-scan on the Lesson/Quiz step or pulse the matching beat caption.
5. **(Optional, cosmetic) Dead recon CSS** — `.ltp-*`/`.chart-view-tabs`/`.cvt-tab`/`.intro-video-*`
   rules linger in `lt-styles.css` (harmless; the v2 player reuses `.recon-lesson-card`/`.recon-player-host`).
