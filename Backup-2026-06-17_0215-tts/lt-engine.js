...
function moduleBannerHtml(chapter, audio) {
  const name = chapter.module;
  if (!name) return '';
  const t = moduleTheme(name);
  // Subtle sound glyph on the right marks a narrated (audio) lesson — shown on the
  // Lesson step only (the animated player), not the text/chart Introduction.
  const audioMark = audio
    ? `<span class="module-banner-audio" title="Narrated lesson" aria-label="Narrated audio lesson"><i data-lucide="volume-2" style="width:16px;height:16px;"></i></span>`
    : '';
...
function renderIntro(chapter) {
  ...
  const chartCard  = hasChart
    ? chartCardHtml('chart-intro', introChart.title, introChart.type === 'line' ? 'Line' : 'Candles', false, introChart.chartHeight, _vurl)
    : '';
  ...
  const rightHtml = chapter.roadmap
    ...
    : hasConcept
      ? conceptCardHtml('concept-intro', introChart, _vurl)
      : chartCard;
