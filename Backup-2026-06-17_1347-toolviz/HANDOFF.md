...
- `Backup-2026-06-17_1036-scenarios/` — **latest** (code only; audio regenerable). Snapshot taken BEFORE
  the scenario/ECharts Ichimoku + RSI-panel pass below. Current cache versions: `lt-engine.js 3.101.9`,
  `lt-styles.css 1.98.3`, `lt-settings.js 1.29.0`, `lessons-v2/chart-vocabulary.js 1.5.0`,
  `lessons-v2/renderer.js 1.13.0`, `lessons-v2/player.css 1.7.2`, `lessons-v2/audio/manifest.js 1.1.0`,
  `lt-module-quizzes.js 1.0.0`, `lt-glossary.js 1.19.0`, `lt-data-course4.js 1.12.0`.
...
### ★ CONTINUE: roll out the Hyblock TOOL VISUALIZATIONS to the rest of the course (owner: "do everything")
The confluence charts NAME tools (Liq Levels / Heatmap / Trading Activity) in text pins but many don't DRAW
them. The rendering system is BUILT in `lt-engine.js` (helpers `_applyLiqCluster`, `_applyHeatmap`,
`_applyActivityPanel` + `_computeCumDelta`/`_computeRSI`; wired in `buildCandlestickOption` via def flags
`liqCluster` / `heatmap` / `tradingActivity`). Already applied + verified on 3 charts (course4 `lt-data-course4.js`):
**Trading Activity intro** (ch idx 23), **Hyblock Platform Overview intro** (idx 18), **Positions Heatmap intro**
(idx 21). **Heatmap colour = Hyblock AMBER `"255,190,40"`** (owner's choice; use this for all heatmaps).
Do the FULL remaining sweep (owner chose "everything"), tuning each tool's placement to that chart's price
path + repointing its text pins onto the visuals, and verifying a sample of each type:
1. **Sentiment sub-panel charts (the genuinely-missing ones — ECharts intro/scenario draw NOTHING):**
   - delta → use `tradingActivity:true`: Cumulative Delta (idx 9), Sentiment Vars (idx 6), Who-Is-In-Control (idx 5).
   - **funding** + **basis** → BUILD a new sub-panel helper (mirror `_applyActivityPanel`; a histogram from a
     synthesized/authored series): Funding Rate (idx 7), Futures Basis (idx 10), Applying Sentiment (idx 11).
   (Note: the v2 LESSONS already show these via the canvas sub-panels — this is only the ECharts intro/scenario.)
2. **Heatmap scenario charts** (short-cluster): Positions Heatmap scenario (idx 21), Combining Sentiment (idx 22) — add amber `heatmap`.
3. **Liq charts that already draw a flat gold band** (idx 19 Liquidation Levels, idx 20 Liq Scenario, idx 1/2/4
   stop-cluster/liquidity-pool charts): upgrade the flat band → a `liqCluster` ladder for consistency.
4. **Course 3:** order-book bid/ask-zone charts (ch3, ch11) need a NEW order-book depth viz; single-liq-level
   charts (ch8/ch9/ch20) mostly already have markLines (leave or convert to `liqCluster` single line).
Full per-chart audit (chapter idx + tool named) is in the dated `later⁶` log entry below. On dual-tool charts
(liq + amber heatmap) consider making the liq a dashed `liqCluster` ladder so it stays distinct from the amber
heatmap gradient. Cache files touched so far: `lt-engine.js 3.101.9`, `lt-data-course4.js 1.12.0`.
