...
// Liquidation-cluster ladder — dashed gold lines at leveraged-liq prices (a magnet
// price runs into). Only the cluster is labelled (so the right-edge pills don't pile).
...
function _applyLiqCluster(opt, def) {
  ...
  const col = lc.color || '#ffcc00', s0 = opt.series[0];
  // faint gold band behind the rungs so the cluster reads as a visible liquidation ZONE
  const lo = Math.min.apply(null, lc.lines), hi = Math.max.apply(null, lc.lines);
  s0.markArea = s0.markArea || { silent: true, data: [] };
  s0.markArea.data.push([{ yAxis: lo - 1, itemStyle: { color: 'rgba(255,204,0,0.10)' } }, { yAxis: hi + 1 }]);
  ...
  lc.lines.forEach((y, i) => {
    ...
    s0.markLine.data.push([
      { yAxis: y,
        label: labelled ? { show: true, formatter: lc.label, color: '#1a1208', fontSize: 9, fontWeight: 700, fontFamily: 'JetBrains Mono, monospace', position: 'end', backgroundColor: col, padding: [2, 6], borderRadius: 4 } : { show: false },
...
function _applyActivityPanel(opt, def, ctx) {
  ...
  opt.series.push({
    name: def.activityLabel || 'Cumulative Delta', type: 'line', data: delta, xAxisIndex: 1, yAxisIndex: 1,
    symbol: 'none', showSymbol: false, silent: true, lineStyle: { width: 2 }, areaStyle: { opacity: 0.16 }, z: 2,
  ...
  // colour the line by sign: red = sellers, teal = buyers
