...
const LT_COURSES = [
  ...
  { num: 3, chapters: (typeof LT_CHAPTERS_3 !== 'undefined' ? LT_CHAPTERS_3 : null),
    exam: (typeof LT_EXAM_QUESTIONS_3 !== 'undefined' ? LT_EXAM_QUESTIONS_3 : []),
  ...
  { num: 4, chapters: (typeof LT_CHAPTERS_4 !== 'undefined' ? LT_CHAPTERS_4 : null),
    exam: (typeof LT_EXAM_QUESTIONS_4 !== 'undefined' ? LT_EXAM_QUESTIONS_4 : []),
...
const courseExamPool = (n) => _courseDef(n).exam;
...
function chapterStepLabels(chapter) { return chapterStepKeys(chapter).map(k => KIND_LABEL[k]); }
...
function _applyIchimokuOverlay(opt, def, ctx) {
  ...
  const aEdge = 'rgba(40,200,120,0.60)', bEdge = 'rgba(242,61,92,0.55)';
  ...
  opt.series.push({ name: '_kumo_bull', type: 'line', data: dBull, stack: 'kumo', symbol: 'none', silent: true, showSymbol: false, lineStyle: { opacity: 0 }, areaStyle: { color: 'rgba(40,200,120,0.20)' }, z: 0, connectNulls: false });
  opt.series.push({ name: '_kumo_bear', type: 'line', data: dBear, stack: 'kumo', symbol: 'none', silent: true, showSymbol: false, lineStyle: { opacity: 0 }, areaStyle: { color: 'rgba(242,61,92,0.18)' }, z: 0, connectNulls: false });
  // faint cloud edges (Senkou A green / Senkou B orange-red)
  opt.series.push({ name: '_spanA', type: 'line', data: ik.spanA, symbol: 'none', silent: true, showSymbol: false, lineStyle: { color: aEdge, width: 1 }, z: 1 });
  opt.series.push({ name: '_spanB', type: 'line', data: ik.spanB, symbol: 'none', silent: true, showSymbol: false, lineStyle: { color: bEdge, width: 1 }, z: 1 });
...
function _applyRsiZonesOverlay(opt, def) {
  ...
  opt.series[0].markArea.data = (opt.series[0].markArea.data || []).concat([
    [{ yAxis: obLevel,
       label: { show: true, formatter: 'Overbought', color: '#ff8fbf', fontSize: 9, fontWeight: 600, position: 'insideTopLeft' },
       itemStyle: { color: 'rgba(' + VIZ.bearRgb + ',0.07)' } },
     { yAxis: maxP + range * 0.02 }],
    [{ yAxis: minP - range * 0.02,
       label: { show: true, formatter: 'Oversold', color: '#7fe6e6', fontSize: 9, fontWeight: 600, position: 'insideTopLeft' },
       itemStyle: { color: 'rgba(' + VIZ.bullRgb + ',0.07)' } },
     { yAxis: osLevel }]
  ]);
}
...
function _applyRsiPanel(opt, def, ctx) {
  ...
  opt.xAxis = [
    ...
    Object.assign({}, baseX, { gridIndex: 1, data: xData,
      ...
      axisLabel: { color: C.TEXT3, fontSize: 9, fontFamily: 'JetBrains Mono, monospace', hideOverlap: true } })
  ];
  ...
  opt.yAxis = [
    ...
    { gridIndex: 1, min: 0, max: 100, interval: 50, scale: false,
      ...
      axisLabel: { color: C.TEXT3, fontSize: 9, fontFamily: 'JetBrains Mono, monospace' } }
  ];
...
function _applyLiqCluster(opt, def) {
  ...
  lc.lines.forEach((y, i) => {
    ...
    s0.markLine.data.push([
      { yAxis: y,
        label: labelled ? { show: true, formatter: lc.label, color: labelTxt, fontSize: 9, fontWeight: 700, fontFamily: 'JetBrains Mono, monospace', position: 'end', backgroundColor: col, padding: [2, 6], borderRadius: 4 } : { show: false },
...
function _applyHeatmap(opt, def) {
  ...
  def.heatmap.forEach(b => {
    ...
    if (b.label) s0.markArea.data.push([
      { yAxis: bestY0, label: { show: true, formatter: b.label, color: '#fff', fontSize: 9, fontWeight: 700, fontFamily: 'JetBrains Mono, monospace', position: 'insideTopLeft', backgroundColor: 'rgba(10,10,12,0.5)', padding: [2, 5], borderRadius: 3 }, itemStyle: { color: 'transparent' } },
      { yAxis: bestY0 + cellH }
    ]);
  });
}
...
function _subPanelGrids(opt, opts) {
  ...
  opt.xAxis = [
    Object.assign({}, baseX, { gridIndex: 0, axisLabel: { show: false }, axisTick: { show: false } }),
    Object.assign({}, baseX, { gridIndex: 1, data: xData, axisLine: { lineStyle: { color: C.BORDER } }, axisTick: { show: false }, axisLabel: { color: C.TEXT3, fontSize: 9, fontFamily: 'JetBrains Mono, monospace', hideOverlap: true } })
  ];
  const baseY = opt.yAxis || {};
  opt.yAxis = [
    Object.assign({}, baseY, { gridIndex: 0 }),
    { gridIndex: 1, scale: opts.min == null, min: opts.min, max: opts.max,
      ...
      axisLabel: opts.fmt ? { show: true, color: C.TEXT2, fontSize: 9, fontFamily: 'JetBrains Mono, monospace', formatter: opts.fmt } : { show: false } }
  ];
  if (opt.tooltip) opt.tooltip.axisPointer = Object.assign({}, opt.tooltip.axisPointer, { link: [{ xAxisIndex: 'all' }] });
  return C;
}
...
function _applyActivityPanel(opt, def, ctx) {
  ...
  opt.series.push({
    ...
    markLine: { symbol: ['none', 'none'], silent: true, data: [
      { yAxis: 0, label: { show: true, formatter: (def.activityLabel || 'Cumulative Δ (CVD)'), position: 'insideStartTop', color: C.TEXT3, fontSize: 9, fontFamily: 'JetBrains Mono, monospace' }, lineStyle: { color: C.BORDER, type: 'dashed', width: 1 } }
    ] }
  });
}
...
function _applyVolumePanel(opt, def, ctx) {
  ...
  opt.series.push({
    ...
    markLine: { symbol: ['none', 'none'], silent: true, data: [
      { yAxis: +avg.toFixed(2), label: { show: true, formatter: (cfg.label || 'Volume') + ' · avg', position: 'insideStartTop', color: C.TEXT3, fontSize: 9, fontFamily: 'JetBrains Mono, monospace' }, lineStyle: { color: C.BORDER, type: 'dashed', width: 1, opacity: 0.6 } }
    ] }
  });
}
...
function _applySubPanel(opt, def, ctx) {
  ...
  if (sp.kind === 'oi' || sp.type === 'line') {
    ...
    opt.series.push({
      ...
      markLine: { symbol: ['none', 'none'], silent: true, data: [
        { yAxis: data[0], label: { show: true, formatter: label, position: 'insideStartTop', color: C.TEXT3, fontSize: 9, fontFamily: 'JetBrains Mono, monospace' }, lineStyle: { color: C.BORDER, type: 'dashed', width: 1, opacity: 0.5 } }
      ] }
    });
  } else {
    opt.series.push({
      ...
      markLine: { symbol: ['none', 'none'], silent: true, data: [
        { yAxis: 0, label: { show: true, formatter: label + hint, position: 'insideStartTop', color: C.TEXT3, fontSize: 9, fontFamily: 'JetBrains Mono, monospace' }, lineStyle: { color: C.BORDER, type: 'dashed', width: 1 } }
      ] }
    });
    opt.visualMap = (opt.visualMap || []).concat([{
      show: false, type: 'piecewise', seriesIndex: si, dimension: 1,
...
function _applyOrderBook(opt, def) {
  ...
  function side(rungs, rgb, label) {
    ...
    rungs.forEach(r => {
      ...
      s0.markLine.data.push([
        { yAxis: r.price,
          label: (isWall && label) ? { show: true, formatter: label, color: '#0a0a0c', backgroundColor: 'rgba(' + rgb + ',0.95)', fontSize: 9, fontWeight: 700, fontFamily: 'JetBrains Mono, monospace', padding: [2, 6], borderRadius: 4, position: 'end' } : { show: false },
...
function _expandChartDef(def) {
  ...
  if ((def.type && def.type !== 'candlestick') || def.format === 'concept' || !Array.isArray(def.ohlc) || !def.ohlc.length) { def.__ctx = true; def.__ctxLead = 0; return 0; }
  const ohlc = def.ohlc;
...
function _expandAllChartContext() {
  ...
  function exam(arr) {
    ...
    arr.forEach(q => {
      if (!q || !q.chart) return;
      const m = _expandChartDef(q.chart);
      if (m && Array.isArray(q.revealMarkPoints)) q.revealMarkPoints.forEach(mp => { if (mp.dataIndex != null) mp.dataIndex += m; });
    });
  }
  ...
  [typeof LT_EXAM_QUESTIONS !== 'undefined' && LT_EXAM_QUESTIONS, typeof LT_EXAM_QUESTIONS_2 !== 'undefined' && LT_EXAM_QUESTIONS_2,
   typeof LT_EXAM_QUESTIONS_3 !== 'undefined' && LT_EXAM_QUESTIONS_3, typeof LT_EXAM_QUESTIONS_4 !== 'undefined' && LT_EXAM_QUESTIONS_4].forEach(exam);
}
...
function _applyIndicatorOverlay(opt, def, ctx) {
  switch (def.indicator) {
    ...
    case 'rsi_zones':                                                 // legacy alias → real panel now
...
function _engTimeAxis(n, tf, seed){
  ...
  const every = Math.max(1, Math.round(n / 7));           // ~7 evenly-spaced visible labels
  return { data, interval: (idx) => idx % every === 0 };
}
...
function buildCandlestickOption(def, revealMode) {
  ...
  // Real date/time axis (replaces generic D1..Dn); custom label arrays (R/%, W/M) pass through.
  const _tf         = def.tf || '1d';
  const _axisSeed   = Math.abs(Math.round((((def.ohlc[0] && (def.ohlc[0][0] + def.ohlc[0][3])) || 1) * 7)) + def.ohlc.length) || 7;
  const _timeAxis   = _engGenericLabels(def.labels) ? _engTimeAxis(ohlc.length, _tf, _axisSeed) : { data: labels, interval: 'auto' };
  ...
  const _opt = {
    ...
    xAxis: {
      ...
      axisTick:  { show: false },
      axisLabel: { color: C.TEXT3, fontSize: sm ? 9 : 10, fontFamily: 'JetBrains Mono, monospace', hideOverlap: true, interval: _timeAxis.interval },
    ...
    yAxis: {
      ...
      axisLabel: { color: C.TEXT2, fontSize: sm ? 9 : 10, fontFamily: 'JetBrains Mono, monospace', formatter: (val) => _engRoundTick(val) }
    },
    series: [{
      name:  'Price',
      type:  'candlestick',
...
function buildLineOption(def) {
  ...
  return {
    ...
    yAxis: {
      ...
      axisLine:  { lineStyle: { color: C.BORDER } },
      axisLabel: { color: C.TEXT2, fontSize: _smLine ? 9 : 10, fontFamily: 'JetBrains Mono, monospace', formatter: (v) => (def.yPrefix || '') + _engRoundTick(v) + (def.ySuffix || '') }
    },
    series
  };
}
...
function renderChart(elId, def, revealMode) {
  ...
  _renderChartLegend(elId, def);   // populate the (off-by-default) colour key for this chart
...
function renderTeachingChart(elId, def) {
  ...
  const mlData = (def.markLines || []).map(ml => [{
    yAxis:     ml.yAxis,
    label:     { show: true, formatter: ml.label, color: _pillText(ml.color || TEAL), fontSize: 10, fontWeight: 700, fontFamily: 'JetBrains Mono, monospace', position: 'end', backgroundColor: ml.color || TEAL, padding: [2, 6], borderRadius: 4 },
    lineStyle: { color: ml.color || TEAL, type: 'dashed', width: 1.5, opacity: 0.8 }
  }, { yAxis: ml.yAxis }]);
  ...
  function runTeachAnimation() {
    ...
    const teachTimer = setInterval(() => {
      ...
      if (count >= total) {
        ...
        setTimeout(() => _deconflictMarkLabels(elId), 80);   // de-overlap callouts once revealed
      }
    }, _REVEAL_MS);
  }
...
function chartCardHtml(id, title, badge, tall, chartHeight, videoUrl) {
  ...
  return `
    <div class="chart-card" style="max-width:720px;margin:0 auto;">
      <div class="chart-card-header">
        <span class="chart-card-title">${title}</span>
        <div class="chart-card-header-right">
          ${headerControls}
          <button class="chart-key-btn" id="key-${id}" onclick="toggleChartLegend('${id}')" title="Show colour key" aria-label="Show colour key" aria-expanded="false"><i data-lucide="key" style="width:13px;height:13px;"></i></button>
      ...
      <div id="legend-${id}" class="chart-legend" role="group" aria-label="Colour key" hidden></div>
      ${videoUrl ? _watchOriginalHtml(videoUrl) : ''}
    </div>`;
}

/* Colour key for a chart — only the swatches actually present on THIS chart, so the key
   stays short and relevant. Candle colours honour the learner's custom bull/bear setting;
   tool colours are the fixed VIZ palette. Toggled by the header "key" button (off by default). */
function _chartLegendItems(def) {
  const items = [], seen = {};
  const add = (c, t) => { if (!seen[t]) { seen[t] = 1; items.push({ c: c, t: t }); } };
  add(TEAL, 'Up candle (bullish)');
  add(BEAR, 'Down candle (bearish)');
  if (def.tradingActivity) { add(VIZ.bull, 'Buying — Δ ≥ 0'); add(VIZ.bear, 'Selling — Δ < 0'); add(VIZ.accent, 'Cumulative delta (CVD)'); }
  if (def.subPanel) {
    const k = def.subPanel.kind;
    if (k === 'funding') { add(VIZ.bull, 'Funding ≥ 0'); add(VIZ.bear, 'Funding < 0 (shorts pay)'); }
    else if (k === 'basis') { add(VIZ.bull, 'Contango (premium)'); add(VIZ.bear, 'Backwardation (discount)'); }
    else add(VIZ.accent, 'Open interest');
  }
  if (def.volume || def.indicator === 'volume') { add(VIZ.bull, 'Up-bar volume'); add(VIZ.bear, 'Down-bar volume'); }
  if (def.liqCluster) add(def.liqCluster.color || VIZ.liq, 'Liquidation cluster');
  if (def.heatmap) add('rgb(' + VIZ.heat + ')', 'Positions heatmap (density)');
  if (def.orderBook) { add(VIZ.bull, 'Bid wall (support)'); add(VIZ.bear, 'Ask wall (resistance)'); }
  if (def.indicator === 'ichimoku') { add('#2bd47a', 'Kumo cloud — bullish'); add('#f23d5c', 'Kumo cloud — bearish'); }
  if (def.indicator === 'rsi' || def.indicator === 'rsi_zones') { add(VIZ.bull, 'RSI line'); add(VIZ.bear, 'Overbought / oversold'); }
  return items;
}
function _renderChartLegend(elId, def) {
  const box = document.getElementById('legend-' + elId);
  const btn = document.getElementById('key-' + elId);
  if (!box) return;
  if (!def || def.type === 'line') { if (btn) btn.style.display = 'none'; box.remove(); return; }   // line charts have no colour code
  if (btn) btn.style.display = '';
  box.innerHTML = _chartLegendItems(def).map(it =>
    '<span class="cl-item"><span class="cl-swatch" style="background:' + it.c + '"></span>' + it.t + '</span>'
  ).join('');
}
window.toggleChartLegend = function (id) {
  const box = document.getElementById('legend-' + id), btn = document.getElementById('key-' + id);
  if (!box) return;
  const show = box.hasAttribute('hidden');
  if (show) box.removeAttribute('hidden'); else box.setAttribute('hidden', '');
  if (btn) { btn.setAttribute('aria-expanded', show ? 'true' : 'false'); btn.classList.toggle('on', show); }
};
...
function ensureV2LessonsLoaded(n) {
  ...
  _v2LoadState[n] = Promise.all(files.map((f) => new Promise((resolve) => {
    ...
    s.src = `lessons-v2/lessons/Course${n}/${f}?v=1.0.2`;
...
function renderV2Lesson(chapter) {
  ...
  ensureV2LessonsLoaded(cn).then(() => {
    if (state.chapter !== _c || state.step !== _s) return;   // navigated away during load
...
/* INTRODUCTION step — overview text on the left, the chapter's chart (or concept diagram /
   course roadmap) on the right, all on one page. (The former separate "Visual" step was
   folded back in here once the recon player was retired.) */
function renderIntro(chapter) {
  ...
  // Right column: roadmap (course-overview chapters) > concept diagram > chart.
  ...
  // A roadmap chapter that ALSO teaches with a chart (the course openers) shows the chart
  // full-width below the overview+roadmap so the Introduction still carries both.
  const belowHtml = (chapter.roadmap && hasChart)
    ? `<div class="intro-chart-below module-themed" style="--mod-accent:${_accent};margin-top:18px;">${chartCard}</div>`
    : '';

  setContent(`
    ${_riskCalloutHtml(chapter)}
    <div class="intro-layout${rightHtml ? '' : ' intro-layout--solo'} module-themed" style="--mod-accent:${_accent}">
...
function _mqQuestionHtml(q, qi, isCleared) {
  ...
  if (q.type === 'multi') {
    ...
    optsHtml = `<div class="mq-options${q.type === 'tf' ? ' mq-options--tf' : ''}">
      ${labels.map((o, oi) => {
        const ok  = correctSet.includes(oi);
...
function _examPool() {
  return courseExamPool(getActiveCourseNum());
}
...
function renderExamQuestion() {
  ...
  if (fill)    fill.style.width = `${Math.round(((idx + 1) / getExamQuestions().length) * 100)}%`;
...
window.showExamAnswerKey = function() {
  ...
  const _examNames = {
    ...
    2: (typeof COURSE2_META !== 'undefined' ? COURSE2_META.title : 'Building Your Toolbox'),
    3: (typeof COURSE3_META !== 'undefined' ? COURSE3_META.title : 'Sharpening Your Edge'),
    4: (typeof COURSE4_META !== 'undefined' ? COURSE4_META.title : 'Liquidity Theory')
  };
  const pools = LT_COURSES.map(c => ({ n: c.num, name: _examNames[c.num], pool: c.exam }));
...
window.showCertificate = function(correct, total) {
  ...
  if (nameEl) {
    nameEl.value = LTStore.get('certName') || '';
    nameEl.oninput = () => { LTStore.set('certName', nameEl.value); };
  }
...
function injectChartStyles() {
  ...
  s.textContent = `
    ...
    .chart-key-btn {
      background: transparent; border: 1px solid #1a1a2e; border-radius: 6px; color: #5a5a78;
      cursor: pointer; display: flex; align-items: center; justify-content: center;
      padding: 3px 4px; flex-shrink: 0; line-height: 0; transition: color 0.2s, border-color 0.2s;
    }
    .chart-key-btn:hover { color: var(--teal, #00d4d4); border-color: var(--teal, #00d4d4); }
    .chart-key-btn.on { color: var(--bg, #08070f); background: var(--teal, #00d4d4); border-color: var(--teal, #00d4d4); }
    .chart-legend {
      display: flex; flex-wrap: wrap; gap: 7px 16px;
      padding: 10px 14px; border-top: 1px solid var(--border, #272235);
      background: var(--bg2, #0d0b18);
      animation: clFade 0.18s var(--ease-out, ease);
    }
    .chart-legend[hidden] { display: none; }
    .cl-item { display: inline-flex; align-items: center; gap: 7px; font-size: 11px; color: var(--text2, #a8a3bb); font-family: 'JetBrains Mono', monospace; line-height: 1.3; }
    .cl-swatch { width: 11px; height: 11px; border-radius: 2px; flex: none; box-shadow: 0 0 5px rgba(0,0,0,0.4) inset; }
    @keyframes clFade { from { opacity: 0; transform: translateY(-3px); } to { opacity: 1; transform: none; } }
    @media (prefers-reduced-motion: reduce) { .chart-legend { animation: none; } }
