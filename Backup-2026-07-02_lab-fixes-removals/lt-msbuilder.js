...
(function () {
  ...
  var PLAY_MS = 650;                     // reveal cadence on Play
  ...
  function _msStyles() {
    LTUtils.injectStyles('lt-msbuilder-styles', [
      ".msb-wrap{width:100%;max-width:640px;margin:0 auto;font-family:'Cascadia Code','JetBrains Mono',ui-monospace,monospace;color:var(--text,#f6f5fb);}",
      ".msb-bar{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:10px;}",
      ".msb-btn{font-family:inherit;font-size:11px;font-weight:700;letter-spacing:.03em;color:var(--text2,#a8a3bb);",
      ...
      ".msb-cndl.shown:hover .msb-hit,.msb-cndl.shown:focus-visible .msb-hit{fill:rgba(255,255,255,.05);}",
      ...
      "@media(max-width:400px){.msb-btn{padding:5px 9px;font-size:10.5px;}.msb-count{width:100%;margin-left:0;order:9;}}"
    ].join(''));
  }
  ...
  function renderMSBuilder(containerId) {
    ...
    el.innerHTML = '<div class="msb-wrap' + (reduce ? ' reduce' : '') + '">'
      + '<div class="msb-bar">'
      + '<button type="button" class="msb-btn primary" id="msb-play">▶ Play</button>'
    ...
    function refreshRead() {
      var r = structureRead();
      if (!r) { setRead('', 'Click the candles you think are <b>swing highs</b> or <b>swing lows</b>. A swing must stand out against ' + CONFIRM + ' candles either side.'); return; }
    ...
    function markSwing(k) {
      ...
      g.innerHTML = '<circle cx="' + X(s.i).toFixed(1) + '" cy="' + (s.side === 'high' ? Y(s.px) - 3.5 : Y(s.px) + 3.5).toFixed(1) + '" r="2.4" fill="' + col + '"/>'
        + '<text x="' + X(s.i).toFixed(1) + '" y="' + ty.toFixed(1) + '" text-anchor="middle" fill="' + col + '" font-size="10" font-weight="700" font-family="JetBrains Mono,monospace">' + s.label + '</text>';
      marks.appendChild(g);
    }
    ...
    function flashMiss(i) {
      ...
      g.innerHTML = '<text x="' + X(i).toFixed(1) + '" y="' + (Y(c.h) - 6).toFixed(1) + '" text-anchor="middle" fill="' + B + '" font-size="11" font-weight="700" font-family="JetBrains Mono,monospace">✕</text>';
      fx.appendChild(g);
    ...
    function onCandleClick(i) {
      ...
      if (pendingK !== -1) {
        setRead('warn', 'Maybe — but a swing only <b>confirms</b> once ' + CONFIRM + ' candles close on its right. Keep playing, then call it.');
        return;
      }
      misses++;
