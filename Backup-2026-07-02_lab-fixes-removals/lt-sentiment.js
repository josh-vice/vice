...
(function () {
  ...
  var STEP_MS = 2600;   // dwell per phase — long enough to read each caption
  ...
  function _sentStyles() {
    ...
    LTUtils.injectStyles('lt-sentiment-styles', [
  ...
  function renderSentimentBoard(containerId) {
    ...
    el._sbTimer = setInterval(function () {
      if (!document.body.contains(el)) { stopSentimentBoard(el); return; }
      i = (i + 1) % PHASES.length;
      paint(i, true);
    }, STEP_MS);
  }
