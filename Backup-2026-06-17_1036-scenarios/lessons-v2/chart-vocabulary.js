...
(function (g) {
  ...
  var CHARTS = {
    ...
    /* ichimoku — the full Ichimoku Kinko Hyo on an uptrend: Tenkan (blue), Kijun
       (yellow), Chikou lagging span (purple), and the Kumo cloud (green bullish /
       red bearish) PROJECTED FORWARD into open space on the right — the leading
       future-S/R cloud that is Ichimoku's signature. Price pulls back to the Kijun
       (~idx 11) and bounces, then runs above a green cloud. Periods scaled for a
       short idealized chart; displacement = the Kijun period. */
    ichimoku: function () {
      var cs = grow(46, [
        { to: 50, bars: 3 }, { to: 44, bars: 2 }, { to: 58, bars: 4 },
        { to: 52, bars: 3, reject: 1.5 },          // pullback taps the Kijun (~idx 11)
        { to: 70, bars: 4 }, { to: 64, bars: 3 }, { to: 84, bars: 5 }
      ]);
      var ik = ichimoku(cs, { conv: 4, base: 9, span: 18, disp: 9 });
      // axis must fit price AND every Ichimoku series (incl. the leading cloud)
      var lo = Infinity, hi = -Infinity;
      for (var i = 0; i < cs.length; i++) { if (cs[i][2] < lo) lo = cs[i][2]; if (cs[i][3] > hi) hi = cs[i][3]; }
      [ik.tenkan, ik.kijun, ik.spanA, ik.spanB, ik.chikou].forEach(function (arr) {
      ...
      return {
        ...
        overlays: [
          ...
          { points: ik.chikou, color: '#b06cff', width: 1.2, label: 'Chikou' }
        ],
        cloud: { a: ik.spanA, b: ik.spanB },
        anchors: {
          pullback: { type: 'point', i: 11, price: 50 },     // price taps the Kijun and bounces
          aboveCloud: { type: 'point', i: cs.length - 1, price: 84 }
        },
