...
function _simStyles() {
  ...
  .sim3-wrap { width:100%; max-width:1180px; margin:0 auto; padding:0 0 40px; font-family:'JetBrains Mono',monospace; }
  ...
  /* controls */
  .sim3-controls { display:flex; flex-wrap:wrap; gap:10px 22px; align-items:center; background:var(--bg3); border:1px solid var(--border); border-radius:var(--radius-lg); padding:10px 14px; margin-bottom:12px; }
  ...
  /* stats strip */
  .sim3-stats { display:flex; gap:10px; margin-bottom:12px; flex-wrap:wrap; }
  .sim3-stat { flex:1; min-width:96px; background:var(--bg3); border:1px solid var(--border); border-radius:var(--radius); padding:9px 14px; }
  ...
  /* main grid */
  .sim3-grid { display:grid; grid-template-columns:1fr 340px; gap:14px; margin-bottom:12px; align-items:stretch; }
  ...
  /* lower grid */
  .sim3-lower { display:grid; grid-template-columns:1fr 1.2fr; gap:12px; margin-bottom:12px; align-items:stretch; }
...
function _renderChart() {
  ...
  _simChart.setOption({
    ...
    xAxis: [
      ...
      { type: 'category', data: labels, gridIndex: 1, axisLine: { lineStyle: { color: bdr } }, axisLabel: { color: '#777', fontSize: 9, fontFamily: 'JetBrains Mono,monospace', hideOverlap: true }, splitLine: { show: false } }
    ],
    yAxis: [
  ...
  const line = (id, price, color, label, draggable, onMove, solidTag) => {
    ...
    return {
      ...
      children: [
        ...
        { type: 'text', name: 't', silent: true, style: { text: label + ' ' + fmt(price), x: tagX + pad, y: -6, fill: solidTag ? '#0b0b0b' : '#e8ebf0', font: '700 10px "JetBrains Mono",monospace' } }
      ]
    };
  };
